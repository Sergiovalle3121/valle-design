...
function assertPlacement(value: unknown, id: string): void {
  ...
  const fields3d = ['m02', 'm12', 'm20', 'm21', 'm22', 'tx', 'ty', 'tz'] as const;
  let has3D = false;
  for (const key of fields3d) {
    if (placement[key] !== undefined) {
      ...
      if (key !== 'tx' && key !== 'ty' && key !== 'tz' && (placement[key] as number) !== 0) has3D = true;
      if ((key === 'tx' || key === 'ty' || key === 'tz') && (placement[key] as number) !== 0) has3D = true;
    }
  }
  if (has3D) {
    // Determinante3×3: a*(d*m22 - m12*m21) - c*(b*m22 - m12*m20) + m02*(b*m21 - d*m20)
    const a = placement.a as number, b = placement.b as number;
    const c = placement.c as number, d = placement.d as number;
    ...
    const det3 = a * (d * m22 - m12 * m21) - c * (b * m22 - m12 * m20) + m02 * (b * m21 - d * m20);
    if (!(Math.abs(det3) > 0)) {
      reject(`el sólido ${id} tiene una colocación3D singular: aplastaría el sólido a un plano.`);
    }
  } else {
    const determinant =
    ...
    if (!(Math.abs(determinant) > 0)) {
      reject(`el sólido ${id} tiene una colocación singular: aplastaría el sólido a un plano.`);
    }
  }
}
