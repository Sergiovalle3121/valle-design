 ...
 */
describePostgres('Guard de entitlement — modo solo-lectura post-expiración', () => {
  ...
  function contextFor(permissions: string[]): {
    ...
    const request: Record<string, unknown> = { user, method: 'GET', path: '/v1/cad/documents' };
    class Handler {}
  ...
  it('3. quien NUNCA contrató sigue viendo el 403 completo, también para leer', async () => {
    ...
    expect(error).toBeInstanceOf(ForbiddenException);
    expect(
      ((error as ForbiddenException).getResponse() as { details: { reason: string } })
        .details.reason,
    ).toBe('not_entitled');
  });
