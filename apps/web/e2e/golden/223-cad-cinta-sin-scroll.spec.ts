 ...
 * rótulo al botón pequeño denso (icono + texto completo, ampliando su ancho
 * según el nombre) a costa de mostrar MENOS: de 71
 ...
 * denso muestra su rótulo entero y conserva su nombre accesible:
...
async function comandosVisibles(
  page: Page,
  tabId: string,
): Promise<{ total: number; scrollWidth: number; clientWidth: number }> {
  return page.evaluate((id) => {
    ...
    const botones = tira.querySelectorAll<HTMLElement>(
      '[data-testid^="cad-ribbon-command-"]',
    );
    return {
      total: botones.length,
      scrollWidth: tira.scrollWidth,
      clientWidth: tira.clientWidth,
    };
  }, tabId);
}
...
test("a 1366×768 Inicio enseña 24 comandos CON rótulo legible o más, sin desplazar la tira (Ola 1 mostraba 71 sin nombre)", async ({
  ...
  expect(
    medida.scrollWidth,
    `la tira mide ${medida.scrollWidth}px en ${medida.clientWidth}px de ventana: se desplazaría`,
  ...
  expect(
    cinta.height,
    "la cinta no cambia de alto con el escalón denso: sigue en ≤108 px",
  ).toBeLessThanOrEqual(108);
});

test("el botón pequeño denso conserva el rótulo (icono + texto) y su nombre accesible", async ({
  context,
  page,
}) => {
  test.setTimeout(120_000);
  ...
  await expect(
    boton,
    "denso CON rótulo: «Girar» está pintado a la vista",
  ).toContainText("Girar");
  await expect(boton, "el atributo que marca el escalón denso").toHaveAttribute(
    "data-dense",
    "true",
  );
  await expect(
    boton,
    "el ancho del botón denso es 5 rem (80 px, Ola 6 «cinta legible»)",
  ...
  // name })` siguen valiendo tal cual.
  const porNombre = page.getByRole("button", { name: "Girar" });
  await expect(
    porNombre,
    "getByRole('button', { name: 'Girar' }) sigue resolviendo",
  ).toHaveCount(1);
  await expect(porNombre).toBeVisible();
  expect(
    await porNombre.getAttribute("data-testid"),
    "es el mismo botón, no un duplicado",
  ).toBe("cad-ribbon-command-ROTATE");
});

test("un rótulo denso se lee entero, sin salirse del botón ni envolver", async ({
  context,
  page,
}) => {
  test.setTimeout(120_000);
  ...
  // rótulos largos del panel: también debe caber completo.
  ...
  expect(
    cajaRotulo.x,
    "el rótulo no empieza antes que su botón",
  ).toBeGreaterThanOrEqual(caja.x - 1);
  expect(
    cajaRotulo.x + cajaRotulo.width,
    "el rótulo no se sale del botón por la derecha",
  ).toBeLessThanOrEqual(caja.x + caja.width + 1);
  expect(
    cajaRotulo.y + cajaRotulo.height,
    "el rótulo cabe en una sola línea: no envuelve ni empuja el alto del botón",
  ).toBeLessThanOrEqual(caja.y + caja.height + 1);
  const texto = await rotulo.evaluate((element) => ({
    requerido: element.scrollWidth,
    disponible: element.clientWidth,
  }));
  expect(
    texto.requerido,
    "el rótulo completo cabe sin elipsis",
  ).toBeLessThanOrEqual(texto.disponible + 1);
  // El title nativo ofrece además la descripción del comando.
