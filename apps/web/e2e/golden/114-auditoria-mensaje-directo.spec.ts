...
class MensajeriaStub {
  ...
  async install(context: BrowserContext) {
    await context.route(`${API_ORIGIN}/v1/messaging/**`, (route) =>
      this.handle(route),
    );
  }
  ...
  private async handle(route: Route) {
    ...
    if (path === "/v1/messaging/channels" && method === "POST") {
      const dto = JSON.parse(route.request().postData() ?? "{}") as Record<
        string,
        unknown
      >;
      ...
      const canal: CanalStub = {
        ...
        otherMember:
          dto.kind === "direct"
            ? {
                userId: dto.memberUserId as string,
                email: "marta@estudio.example",
    ...
    if (/^\/v1\/messaging\/channels\/[^/]+\/read$/.test(path))
...
class PresenciaServidorStub {
  ...
  async install(context: BrowserContext) {
    await context.route(`${API_ORIGIN}/v1/cad/documents/*/presence*`, (route) =>
      this.handle(route),
    );
    await context.route(
...
test("escéptico: con la socia presente, ¿algún control de TODO el estudio abre una conversación con ELLA?", async ({
  ...
  const page = await abrirEstudio(context, backendConProyecto(), {
    mensajeria,
    presencia,
  });
  ...
  await expect(page.getByTestId("team-messaging-draft")).toBeVisible({
    timeout: 30_000,
  });

  // Inventario de TODA la página, no sólo del panel. Se adjunta entero al
  // informe de Playwright para revisarlo a mano: el catálogo de mobiliario
  // mete ruido («Persona», descripciones de plantillas) y un regex ciego lo
  // confundiría con un mando de mensajería. (Nació escribiendo a una ruta
  // absoluta del contenedor de la auditoría, que en CI no existe.)
  const inventario = await page.evaluate(() =>
    Array.from(
      ...
      dentroDeMuelle: !!el.closest(
        '[data-testid="team-messaging-dock"], [data-testid="cad-collab-dock"]',
      ),
    })),
  );
  await testInfo.attach("inventario-de-controles", {
    body: JSON.stringify(inventario, null, 2),
    contentType: "application/json",
  });
  console.log(
    `[esceptico] controles totales en la página: ${inventario.length}`,
  );
  const enMuelles = inventario.filter((c) => c.dentroDeMuelle);
  console.log(
    `[esceptico] controles DENTRO de los dos muelles de colaboración: ${JSON.stringify(enMuelles, null, 2)}`,
  );
  ...
  console.log(
    `[esceptico] la insignia de la socia es pulsable: ${insigniaEsBoton}`,
  );

  await page.waitForTimeout(2_000);
  const directos = mensajeria.creaciones.filter((c) => c.kind === "direct");
  console.log(
    `[esceptico] canales pedidos al servidor: ${JSON.stringify(mensajeria.creaciones)}`,
  );
  expect(
  ...
  expect(insigniaEsBoton, "la insignia de la socia tampoco es pulsable").toBe(
    false,
  );
...
test("escéptico: si el servidor YA devuelve un canal directo, el panel lo pinta y deja escribir", async ({
  ...
  const canalDirecto = page.getByTestId(
    "team-messaging-channel-30000000-0000-4000-8000-0000000000aa",
  );
  await expect(
    canalDirecto,
    "el panel SÍ sabe pintar un canal directo",
  ).toBeVisible({ timeout: 30_000 });
  await expect(canalDirecto).toContainText("Marta Ruiz");
  await canalDirecto.click();
  await page
    .getByTestId("team-messaging-draft")
    .fill("Marta, mira el muro sur.");
  ...
  console.log(
    `[esceptico] rótulos con canal directo preexistente: ${JSON.stringify(rotulos)}`,
  );
});
