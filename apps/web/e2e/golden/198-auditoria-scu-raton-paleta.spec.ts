...
test("LINE desde la paleta, dos clics sobre la fachada con el SCU apoyado en ella: el trazo queda EN la fachada (T-52)", async ({
  ...
  // El atajo nativo exige plegar la línea de comandos. La volvemos a abrir
  // desde el espacio de trabajo antes de consultar su registro.
  await page.getByTestId("cad-rail-workspace").click();
  await page.getByTestId("cad-workspace-commandDock").check();
  await page.getByLabel("Cerrar panel profesional").click();
  await page.getByTestId("cad-command-log-toggle").click();
  ...
  for (const [i, p] of [linea!.start, linea!.end].entries()) {
    expect(
      p.y,
      `punto ${i + 1} (${p.x}, ${p.y}, ${p.z}) debería caer en la fachada y=${Y1}`,
    ).toBeCloseTo(Y1, 3);
    expect(
      p.z ?? 0,
      `punto ${i + 1} no puede quedar aplanado a cota 0`,
    ).toBeGreaterThan(1);
  }
});
