...
const cadDocument = {
  ...
  history: [], modelSpace: { entityIds: ['snap-horizontal'] }, paperSpaces: [],
...
test('apagar un modo OSNAP en DSETTINGS deja de capturar por él', async ({ context, page }, testInfo) => {
  ...
  await expect(page.getByTestId('cad-draft-settings-mode-count')).toHaveText('12/14');
  await page.getByTestId('cad-draft-settings').screenshot({ path: testInfo.outputPath('draft-settings.png'), scale: 'css' });
  ...
  await test.step('el cuadro y la barra de estado son el mismo estado', async () => {
    ...
    await page.getByTestId('cad-draft-settings-close').click();
    await expect(page.getByTestId('cad-draft-status-osnap')).toHaveAttribute('data-active', 'false');
  ...
  await test.step('«ninguno» y «por defecto» actúan sobre los catorce', async () => {
    ...
    await expect(page.getByTestId('cad-draft-settings-mode-count')).toHaveText('14/14');
