...
function canonicalDocument(): CadDocument {
  return {
    ...
    entities: [
      { id: 'style-line', type: 'line', start: { x: 1_000, y: 1_000, z: 0 }, end: { x: 6_000, y: 1_000, z: 0 }, layer: '0' },
      {
        id: 'style-note', type: 'mtext', text: 'Nota de proceso',
        insertion: { x: 2_000, y: 3_000, z: 0 }, height: 120, width: 3_000, rotation: 0,
        style: 'Titulos', layer: '0',
      },
    ],
    history: [], modelSpace: { entityIds: ['style-line', 'style-note'] }, paperSpaces: [],
    styles: { text: { Titulos: {} }, dimension: {}, mleader: {}, table: {}, plot: {} },
...
test('el gestor define estilos de texto, cota y ploteo, y se niega a borrar uno en uso', async ({ context, page }, testInfo) => {
  ...
  await test.step('un estilo en uso no se borra', async () => {
    ...
    await expect(page.getByTestId('cad-style-usage-Titulos')).toHaveText('1 uso(s)');
  ...
  await test.step('editar el estilo de texto que ya existe', async () => {
    ...
    await expect(page.getByTestId('cad-style-field-Titulos-height')).toHaveValue('120');
  ...
  await test.step('crear un estilo de cota', async () => {
    ...
    await expect(page.getByTestId('cad-style-usage-Arquitectura')).toHaveText('0 uso(s)');
    ...
    // aunque no se pulse.
    await expect(page.getByTestId('cad-style-delete-Arquitectura')).toBeEnabled();
  });
  ...
  await test.step('el enum de ploteo', async () => {
    ...
    await page.getByTestId('cad-style-create').click();
    await page.getByTestId('cad-style-field-Blanco y negro-colorMode').selectOption('monochrome');
    await expect(page.getByTestId('cad-style-field-Blanco y negro-colorMode')).toHaveValue('monochrome');
  });

  await page.getByTestId('cad-style-close').click();
  await saveAndSettle(page, backend);
  const stored = backend.snapshot().document;
  expect(stored.styles.text.Titulos).toEqual({ height: 250, fontFamily: 'Inter' });
  expect(stored.styles.dimension.Arquitectura).toEqual({ precision: 0 });
  expect(stored.styles.plot['Blanco y negro']).toEqual({ colorMode: 'monochrome' });
  ...
  expect(stored.history.some((entry) => entry.label === 'style:create:dimension:Arquitectura')).toBe(true);
  expect(stored.history.some((entry) => entry.label === 'style:update:text:Titulos')).toBe(true);

  await page.reload();
  await expect(page.getByTestId('cad-native-entity-style-line')).toBeVisible();
  await page.getByTestId('cad-draft-status-styles').click();
  await expect(page.getByTestId('cad-style-field-Titulos-height')).toHaveValue('250');
  await page.getByTestId('cad-style-manager').screenshot({ path: testInfo.outputPath('style-manager.png'), scale: 'css' });
});
