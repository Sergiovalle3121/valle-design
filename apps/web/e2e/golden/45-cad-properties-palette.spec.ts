...
function canonicalDocument(): CadDocument {
  return {
    ...
    history: [],
    modelSpace: {
      entityIds: ["multi-circle-a", "multi-circle-b", "multi-arc"],
    },
    paperSpaces: [],
...
test("la paleta de propiedades edita N objetos a la vez y marca lo que difiere", async ({
  ...
  await test.step("un solo objeto: la paleta enseña todas sus propiedades", async () => {
    ...
    await expect(page.getByTestId("cad-native-property-diameter")).toHaveValue(
      "600",
    );
    // Las categorías no son decoración: separan la capa de la geometría.
    await expect(
      page.getByTestId("cad-properties-group-General"),
    ).toBeVisible();
    await expect(
      page.getByTestId("cad-properties-group-Geometría"),
    ).toBeVisible();
  });
  ...
  await test.step("dos círculos: lo común se edita, lo divergente se marca", async () => {
    ...
    await expect(page.getByTestId("cad-native-property-centerY")).toHaveValue(
      "2000",
    );
    ...
    await expect(
      page.getByTestId("cad-native-property-radius"),
    ).not.toHaveAttribute("data-varies", "true");
  });
  ...
  await test.step("tres tipos: sólo se ofrece la INTERSECCIÓN", async () => {
    ...
    await expect(page.getByTestId("cad-native-property-diameter")).toHaveCount(
      0,
    );
    await expect(
      page.getByTestId("cad-native-property-startAngle"),
    ).toHaveCount(0);
    await expect(page.getByTestId("cad-native-property-layer")).toHaveAttribute(
      "data-varies",
      "true",
    );
  ...
  const circleA = stored.entities.find(
    (entity) => entity.id === "multi-circle-a",
  ) as CadCircle;
  const circleB = stored.entities.find(
    (entity) => entity.id === "multi-circle-b",
  ) as CadCircle;
  const arc = stored.entities.find(
    (entity) => entity.id === "multi-arc",
  ) as CadArc;
  ...
  expect([circleA.layer, circleB.layer, arc.layer]).toEqual([
    "STRUCTURE",
    "STRUCTURE",
    "STRUCTURE",
  ]);
  ...
  expect(
    stored.history.filter((entry) => entry.label === "properties:2"),
  ).toHaveLength(1);
  expect(
    stored.history.filter((entry) => entry.label === "properties:3"),
  ).toHaveLength(1);
  // Y no queda rastro de la vía antigua, que aplicaba los comandos de uno en
  // uno y dejaba una entrada por entidad.
  expect(
    stored.history.filter((entry) => entry.label === "properties:circle"),
  ).toHaveLength(0);
  ...
  await page
    ...
    .screenshot({
      path: testInfo.outputPath("properties-palette.png"),
      scale: "css",
    });
});
