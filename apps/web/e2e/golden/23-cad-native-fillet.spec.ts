  ...
  return {
    ...
    entities: [
      ...
      },
      {
        ...
        type: "line",
        start: { x: 4_000, y: 4_000, z: 0 },
        end: { x: 4_000, y: 8_000, z: 0 },
        layer: "WALLS",
      },
    ],
    history: [],
    modelSpace: { entityIds: ["fillet-horizontal", "fillet-vertical"] },
    paperSpaces: [],
...
test("FILLET trims two LINE entities and persists a tangent semantic ARC atomically", async ({
  context,
  page,
}, testInfo) => {
  ...
  await expect(
    page.getByTestId("cad-native-entity-fillet-horizontal"),
  ).toBeVisible();
  ...
  await expect(page.getByTestId("cad-native-property-centerX")).toHaveValue(
    "4500",
  );
  ...
  const arcId = backend
    .snapshot()
    .document.entities.find((entity) => entity.type === "arc")?.id;
  ...
  const storedArc = stored.entities.find(
    (entity): entity is CadArc => entity.type === "arc",
  );
  expect(storedArc).toBeDefined();
  expect(storedArc?.radius).toBe(500);
  expect(storedArc?.context?.metadata).toMatchObject({
    sourceType: "FILLET",
    sourceIds: "fillet-horizontal,fillet-vertical",
  });
  expect(
    stored.history.some((change) => change.label.includes("fillet:")),
  ).toBe(true);
  expect(
    stored.entities.find((entity) => entity.id === "fillet-horizontal"),
  ).toMatchObject({ start: { x: 4_500, y: 4_000 } });
  expect(
    stored.entities.find((entity) => entity.id === "fillet-vertical"),
  ).toMatchObject({ start: { x: 4_000, y: 4_500 } });
  ...
  const importedArc = imported.primitives.find(
    (primitive) => primitive.kind === "arc",
  );
  expect(importedArc?.radius).toBe(500);
  await page.screenshot({
    path: testInfo.outputPath("native-fillet.png"),
    fullPage: true,
    scale: "css",
  });
});
