...
function canonicalDocument(): CadDocument {
  return {
    ...
    layers: [
      ...
      { id: 'NOTES', name: 'NOTES', color: '#22c55e', visible: true, locked: false, linetype: 'CONTINUOUS', lineweight: -1, plot: true },
    ],
    entities: [
      { id: 'layer-base', type: 'line', start: { x: 1_000, y: 1_000, z: 0 }, end: { x: 9_000, y: 1_000, z: 0 }, layer: '0' },
      { id: 'layer-beam', type: 'line', start: { x: 1_000, y: 4_000, z: 0 }, end: { x: 9_000, y: 4_000, z: 0 }, layer: 'STRUCTURE' },
    ],
    history: [], modelSpace: { entityIds: ['layer-base', 'layer-beam'] }, paperSpaces: [],
...
test('el gestor de capas escribe tipo de línea, grosor y plot, filtra, y guarda y restaura estados', async ({ context, page }, testInfo) => {
  ...
  await expect(page.getByTestId('cad-layer-vpfreeze-STRUCTURE')).toHaveAttribute('data-state', 'unavailable');
  ...
  await test.step('tipo de línea, grosor y plot', async () => {
    ...
    await expect(page.getByTestId('cad-layer-plot-NOTES')).toHaveAttribute('data-active', 'false');
  });
  ...
  await test.step('los dos filtros se acumulan', async () => {
    ...
    await expect(page.getByTestId('cad-layer-visible-count')).toHaveText('1/3 listadas');
    ...
    await expect(page.getByTestId('cad-layer-visible-count')).toHaveText('0/3 listadas');
    await page.getByTestId('cad-layer-filter-clear').click();
    await expect(page.getByTestId('cad-layer-visible-count')).toHaveText('3/3 listadas');
  });
  ...
  await test.step('un estado de capa devuelve el reparto anterior', async () => {
    ...
    await expect(page.getByTestId('cad-layer-visible-NOTES')).toHaveClass(/opacity-30/);
    await page.getByTestId('cad-layer-state-restore-Impresion').click();
    await expect(page.getByTestId('cad-layer-visible-NOTES')).not.toHaveClass(/opacity-30/);
  });
  ...
  expect(stored.history.some((entry) => entry.label === 'layer:update:STRUCTURE')).toBe(true);
  expect(stored.history.some((entry) => entry.label === 'layer:update:NOTES')).toBe(true);

  await page.reload();
  await expect(page.getByTestId('cad-native-entity-layer-base')).toBeVisible();
  await viewButton.click();
  await expect(page.getByTestId('cad-layer-linetype-STRUCTURE')).toHaveValue('DASHED');
  await expect(page.getByTestId('cad-layer-lineweight-STRUCTURE')).toHaveValue('0.5');
  await expect(page.getByTestId('cad-layer-plot-NOTES')).toHaveAttribute('data-active', 'false');
  ...
  await page.getByTestId('cad-layer-manager').screenshot({ path: testInfo.outputPath('layer-properties.png'), scale: 'css' });
});

test('congelar una capa en un viewport no la apaga en el resto del dibujo', async ({ context, page }) => {
  test.setTimeout(180_000);
  await installMockBackend(context);
  await loginAsStandaloneOwner(context);
  const backend = await installCadBackend(context);
  ...
  await freeze.click();
  await expect(page.getByTestId('cad-layer-vpfreeze-STRUCTURE')).toHaveAttribute('data-state', 'frozen');
  // Congelada en el viewport, ENCENDIDA en el dibujo: son cosas distintas y el
  // gestor no las confunde.
  await expect(page.getByTestId('cad-layer-visible-STRUCTURE')).not.toHaveClass(/opacity-30/);
  await viewButton.click();

  await saveAndSettle(page, backend);
  const stored = backend.snapshot().document;
  const viewports = stored.paperSpaces.flatMap((space) => space.viewports ?? []);
  const frozen = viewports.filter((viewport) => viewport.layerVisibility?.STRUCTURE === false);
  expect(frozen).toHaveLength(1);
  expect(viewports.length).toBeGreaterThan(1);
  expect(stored.layers.find((layer) => layer.id === 'STRUCTURE')?.visible).toBe(true);
