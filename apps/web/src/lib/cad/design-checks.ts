  ...
  const clean = (arr: CheckBox[] | undefined): CheckBox[] =>
    (Array.isArray(arr) ? arr : [])
      ...
      .map((b) => ({
        ...
        x: fin(b.x),
        y: fin(b.y),
        w: Math.max(0, fin(b.w)),
        h: Math.max(0, fin(b.h)),
      }));
  ...
  items.push(
    unplaced > 0
      ? {
          key: "unplaced",
          level: "warn",
          label: "Estaciones sin colocar",
          detail: `${unplaced} estación(es) siguen en la bandeja, fuera del plano.`,
          count: unplaced,
        }
      : {
          key: "unplaced",
          level: "ok",
          label: "Estaciones colocadas",
          detail: "Todas las estaciones están en el plano.",
          count: 0,
        },
  );
  ...
  items.push(
    outside.length > 0
      ? {
          key: "bounds",
          level: "error",
          label: "Objetos fuera de la huella",
          detail: `${outside.length} objeto(s) se salen del plano: ${outside
            .slice(0, 3)
            .map((b) => b.label)
      ...
      : {
          key: "bounds",
          level: "ok",
          label: "Dentro de la huella",
          detail: "Todos los objetos caben en el plano.",
          count: 0,
        },
  );
    ...
    for (let i = 0; i < all.length; i++) {
      for (let j = i + 1; j < all.length; j++) {
        if (overlaps(all[i], all[j], eps)) {
          overlapCount++;
          if (overlapLabels.length < 3)
            overlapLabels.push(`${all[i].label}↔${all[j].label}`);
        }
      }
    }
  }
  items.push(
    overlapCount > 0
      ? {
          key: "overlap",
          level: "warn",
          label: "Posibles traslapes",
          detail: `${overlapCount} par(es) se traslapan: ${overlapLabels.join(", ")}${overlapCount > 3 ? "…" : ""}.`,
          count: overlapCount,
        }
      : {
          key: "overlap",
          level: "ok",
          label: "Sin traslapes",
          detail: "Ningún objeto se encima con otro.",
          count: 0,
        },
  );
