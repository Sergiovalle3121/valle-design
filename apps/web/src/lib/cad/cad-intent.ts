 ...
 *
 * Correr tests:  npx tsx src/lib/cad/cad-intent.spec.ts
 */
...
export const CAD_TOOLS: CadTool[] = [
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "Analiza y propone correcciones de geometría: duplicados, muros casi ortogonales y segmentos muy cortos. Nunca aplica sin confirmación.",
      parameters: {
        type: "object",
        properties: {
...
export type NormalizeResult =
  { ok: true; intent: CadIntent } | { ok: false; error: string };
  ...
  switch (name) {
    case "setFootprint": {
      ...
      if (w === null || h === null || w <= 0 || h <= 0)
        return { ok: false, error: "footprint inválido" };
    ...
    case "placeAsset": {
      ...
      const kind = String(args.kind ?? "")
        ...
        .toLowerCase();
      if (x === null || y === null)
        return { ok: false, error: "placeAsset requiere x,y numéricos" };
      if (!ASSET_KINDS.includes(kind as (typeof ASSET_KINDS)[number]))
        return { ok: false, error: `kind desconocido: ${kind || "(vacío)"}` };
      return {
        ok: true,
        intent: {
          kind: "placeAsset",
    ...
    case "addDimension": {
      ...
      if (x1 === null || y1 === null || x2 === null || y2 === null)
        return { ok: false, error: `${name} requiere x1,y1,x2,y2` };
      return {
        ok: true,
        intent: {
          kind: "draw",
    ...
    case "moveStation": {
      ...
      if (!station)
        return {
          ok: false,
          error: "moveStation requiere el nombre de la estación",
        };
      if (x === null || y === null)
        return { ok: false, error: "moveStation requiere x,y numéricos" };
      return {
        ok: true,
        intent: { kind: "moveStation", station: station.slice(0, 64), x, y },
      };
    }
    case "cleanupGeometry": {
      ...
      if (tolerance !== null && tolerance <= 0)
        return {
          ok: false,
          error: "cleanupGeometry requiere tolerancia positiva",
        };
      return {
        ok: true,
        intent: {
