 ...
 *
 * Correr tests:  npx tsx src/lib/cad/cad-command.spec.ts
 */
  ...
  switch (state.id) {
    case "line": {
      ...
      return {
        ...base,
        points: [b],
        emitted: [{ type: "addSegment", a, b }],
        prompt: "Siguiente punto (Enter para terminar)",
      };
    }
    case "polyline": {
      ...
      return {
        ...
        prompt:
          pts.length < 2
            ? "Siguiente punto"
            : "Siguiente punto (Enter para terminar)",
      };
    }
    case "rect": {
    ...
    case "circle": {
      if (pts.length < 2)
        return {
          ...base,
          prompt: "Radio o punto del perímetro",
          awaitingRadius: true,
        };
      ...
      return {
        ...
        awaitingRadius: false,
        emitted: [{ type: "addCircle", cx: pts[0].x, cy: pts[0].y, r }],
      };
    }
    case "move":
    case "copy": {
...
export function feedDistance(state: CommandState, d: number): CommandState {
  ...
  if (state.id === "offset") {
    return {
      ...
      emitted: [{ type: "offsetBy", distance: d }],
    };
  }
...
export function commit(state: CommandState): CommandState {
  if (state.id === "polyline" && state.points.length >= 2) {
    return {
      ...
      emitted: [{ type: "addPolyline", points: state.points, closed: false }],
    };
  }
...
export function closePolyline(state: CommandState): CommandState {
  ...
  return {
    ...
    emitted: [{ type: "addPolyline", points: state.points, closed: true }],
  };
}
