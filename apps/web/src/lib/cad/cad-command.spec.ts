/** Tests de cad-command (Fase 66/67). npx tsx src/lib/cad/cad-command.spec.ts */
import {
  startCommand,
  feedPoint,
  feedDistance,
...
// ── circle: centro + punto perímetro → radio por distancia ──
{
  ...
  const a = first(s.emitted);
  ok(
    s.done && a.type === "addCircle" && near(a.r, 5),
    "circle radio = distancia (5)",
  );
}
...
// ── circle: radio tecleado por número ──
{
  ...
  const a = first(s.emitted);
  ok(
    s.done && a.type === "addCircle" && near(a.r, 7) && near(a.cx, 1),
    "circle radio tecleado",
  );
}
...
// ── line: encadena, emite un tramo por punto y reancla ──
{
  ...
  s = feedPoint(s, { x: 0, y: 0 });
  ok(
    s.emitted.length === 0 && s.prompt === "Siguiente punto",
    "line: primer punto no emite",
  );
  ...
  ok(
    s.points.length === 1 &&
      near((s.points[0] as { x: number; y: number }).x, 10),
    "line: reancla en último punto",
  );
  s = feedPoint(s, { x: 10, y: 5 });
  const b = first(s.emitted);
  ok(
    b.type === "addSegment" && near(b.a.x, 10) && near(b.b.y, 5),
    "line: emite tramo 2 encadenado",
  );
}
...
// ── polyline: acumula y emite en commit ──
{
  ...
  ok(
    s.done &&
      ...
      a.points.length === 3 &&
      a.closed === false,
    "polyline emite 3 puntos en commit",
  );
}
...
// ── polyline: Close conecta el último vértice con el primero ──
{
  ...
  const a = first(s.emitted);
  ok(
    s.done && a.type === "addPolyline" && a.closed === true,
    "polyline cierra explícitamente",
  );
}
...
// ── move: base + destino → delta ──
{
  ...
  const a = first(s.emitted);
  ok(
    s.done && a.type === "moveBy" && near(a.dx, 30) && near(a.dy, -10),
    "move delta (+30,-10)",
  );
}
...
// ── copy: emite copyBy ──
{
  ...
  const a = first(s.emitted);
  ok(
    a.type === "copyBy" && near(a.dx, -5) && near(a.dy, 5),
    "copy delta (-5,+5)",
  );
}
...
// ── offset por distancia ──
{
  ...
  const a = first(s.emitted);
  ok(
    s.done && a.type === "offsetBy" && near(a.distance, 2.5),
    "offset distancia 2.5",
  );
}
...
// ── cancel limpia ──
{
  ...
  s = cancel(s);
  ok(
    s.done && s.emitted.length === 0 && s.points.length === 0,
    "cancel limpia sin emitir",
  );
}
...
// ── preview rubber-band ──
{
  ...
  ok(
    pv !== null && pv.type === "addRect" && near(pv.w, 4) && near(pv.h, 3),
    "preview rect bajo cursor",
  );
}
{
