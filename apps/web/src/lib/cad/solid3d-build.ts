...
/**
 * Lleva un plano de coordenadas del mundo a coordenadas locales del sólido.
 *
...
export function cadSolidWorldPlaneToLocal(
  ...
  // Inversa de la3×3: adj(A)/det(A). Para matrices ortogonales (rotación +
  // escala uniforme) la traspuesta de la inversa es la propia inversa, así que
  // la normal se transforma igual que el origen.
  const det3 =
    m.a * (m.d * m.m22 - m.m12 * m.m21) -
    m.c * (m.b * m.m22 - m.m12 * m.m20) +
    m.m02 * (m.b * m.m21 - m.d * m.m20);
  if (Math.abs(det3) < 1e-12) return plane; // Singular: no transformar.
  const invDet = 1 / det3;
  // Adjugada (traspuesta de la cofactora).
  ...
  const localNormal = {
    x: localNormalRaw.x / nLen,
    y: localNormalRaw.y / nLen,
    z: localNormalRaw.z / nLen,
  };
  return { origin: localOrigin, normal: localNormal };
}
