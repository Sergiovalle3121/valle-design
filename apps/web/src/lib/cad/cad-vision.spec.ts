/** Tests de cad-vision (Fase 71). npx tsx src/lib/cad/cad-vision.spec.ts */
...
// ── system prompt ──
ok(
  VISION_SYSTEM_PROMPT.includes("walls") &&
    VISION_SYSTEM_PROMPT.includes("[0,1]"),
  "prompt describe el JSON y la normalización",
);
...
// ── muro normalizado → footprint, con flip Y ──
{
  ...
  ok(
    near(r.walls[0].a.x, 0) && near(r.walls[0].a.y, 5000),
    "esquina sup-izq → (0,H)",
  );
  ok(
    near(r.walls[0].b.x, 10000) && near(r.walls[0].b.y, 5000),
    "esquina sup-der → (W,H)",
  );
}
...
{
  const r = normalizeVision(
    { walls: [{ x1: -0.5, y1: 0.5, x2: 2, y2: 0.5 }] },
    FP,
  );
  ok(
    near(r.walls[0].a.x, 0) && near(r.walls[0].b.x, 10000),
    "x se clampa a [0,1]→[0,W]",
  );
}
...
// ── muro inválido / longitud cero se descarta con error ──
{
  ...
  ok(
    r.walls.length === 0 && r.errors.length === 2,
    "descarta muro cero y muro con coord inválida",
  );
}
...
// ── zona con ≥3 puntos ──
{
  ...
  ok(
    r.zones.length === 1 &&
      r.zones[0].name === "ESD" &&
      r.zones[0].points.length === 3,
    "zona ESD con 3 puntos",
  );
}
...
// ── zona con <3 puntos se descarta ──
{
  ...
  ok(
    r.zones.length === 0 && r.errors.some((e) => e.includes("zona")),
    "zona con 2 puntos → descartada",
  );
}
...
}
{
  ...
  ok(
    r.walls.length === 0 && r.errors.length === 1,
    "JSON inválido → error, sin crash",
  );
}
