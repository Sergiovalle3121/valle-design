...
function documentWith(entities: CadEntity[]): CadDocument {
  return migrateCadDocument({
    ...
    layers: [
      { id: layer, name: "Muros", color: "#fff", visible: true, locked: false },
    ],
    entities,
...
function makeContext(
  ...
  return {
    ...
    entity: (entityId) =>
      document.entities.find((entity) => entity.id === entityId),
    selection,
    activeLayer: layer,
...
function soleSolid(document: CadDocument) {
  const solids = document.entities.filter(
    (entity) => entity.type === "solid3d",
  );
  assert.equal(
    solids.length,
    1,
    `se esperaba UN sólido y hay ${solids.length}`,
  );
...
const keyword = (value: string): CadCommandInput => ({
  kind: "keyword",
  keyword: value,
});
const distance = (value: number): CadCommandInput => ({
  kind: "distance",
  value,
});
const angle = (degrees: number): CadCommandInput => ({
  kind: "angle",
  degrees,
});
...
// una región» y «extruirla» serían dos mundos que no se tocan.
{
  ...
  const afterRegion = apply("REGION", [select("rect"), ENTER], document, [
    "rect",
  ]);
  const next = apply(
    "EXTRUDE",
  ...
  near(
    solid3dMassProperties(soleSolid(next)).volume,
    24_000_000,
    "volumen de lo extruido tras REGION",
    1e-3,
  );
}
...
{
  const document = documentWith([rectangle("rect", 0, 0, 400, 300)]);
  const next = apply("EXTRUDE", [select("rect"), distance(200)], document, [
    "rect",
  ]);
  const solid = soleSolid(next);
  near(
    solid3dMassProperties(solid).volume,
    24_000_000,
    "volumen de la extrusión",
    1e-3,
  );
  assert.ok(
    !next.entities.some((entity) => entity.id === "rect"),
    "el perfil se consume",
  );
  ...
  assert.match(
    messageOf(
      run("EXTRUDE", [select("rect"), distance(0)], document, ["rect"]),
    ),
  ...
  near(
    (taperedNode.draftAngleRad ?? 0) * (180 / Math.PI),
    10,
    "el ángulo se guarda en radianes",
  );
  ...
  assert.match(
    messageOf(
      run(
        "EXTRUDE",
        [select("rect"), keyword("Inclinación"), angle(120)],
        document,
        ["rect"],
      ),
    ),
...
// --- PRESSPULL ----------------------------------------------------------------
{
  ...
  const next = apply("PRESSPULL", [select("rect"), distance(50)], document, [
    "rect",
  ]);
  near(
    solid3dMassProperties(soleSolid(next)).volume,
    500_000,
    "volumen de PRESSPULL",
    1e-3,
  );
}
...
// --- REVOLVE ------------------------------------------------------------------
{
  ...
  const next = apply(
    "REVOLVE",
    [select("perfil"), point(0, 0), point(0, 10), ENTER],
    document,
    ["perfil"],
  );
  const solid = soleSolid(next);
  ...
  near(
    solid3dMassProperties(solid).volume,
    expected,
    "volumen de la revolución facetada",
    1e-3,
  );
  assert.ok(
    validateBody(solid3dBody(solid), {
      requireClosed: true,
  ...
  assert.equal(
    eulerCounts(solid3dBody(solid)).genus,
    1,
    "un tubo tiene género 1",
  );
  ...
  assert.match(
    messageOf(
      run(
        "REVOLVE",
        [select("cruza"), point(0, 0), point(0, 10), ENTER],
        crossing,
        ["cruza"],
      ),
    ),
...
// --- SWEEP --------------------------------------------------------------------
{
  ...
  const next = apply("SWEEP", [select("seccion", "guia"), ENTER], document, [
    "seccion",
    "guia",
  ]);
  const solid = soleSolid(next);
  ...
  assert.ok(
    !next.entities.some((entity) => entity.id === "guia"),
    "el camino se consume",
  );
...
// --- LOFT ---------------------------------------------------------------------
{
  ...
  const next = apply("LOFT", [select("cima", "base"), ENTER], document, [
    "cima",
    "base",
  ]);
  const solid = soleSolid(next);
  ...
  assert.ok(
    validateBody(solid3dBody(solid), { requireClosed: true }).ok,
    "el solevado es un sólido válido",
  );

  const sameHeight = documentWith([
    rectangle("a", 0, 0, 10, 10, 5),
    rectangle("b", 0, 0, 20, 20, 5),
  ]);
  assert.match(
...
// --- UNION / SUBTRACT / INTERSECT ---------------------------------------------
{
  ...
  near(
    solid3dMassProperties(unionSolid).volume,
    175_000,
    "volumen de la unión",
    1e-3,
  );
  assert.equal(
    unionSolid.nodes.length,
    3,
    "el árbol conserva los dos operandos MÁS la operación",
  );
  assert.ok(
  ...
  const subtracted = apply(
    "SUBTRACT",
    [select(firstId, secondId), ENTER],
    document,
    [firstId, secondId],
  );
  near(
    solid3dMassProperties(soleSolid(subtracted)).volume,
    75_000,
    "volumen de la resta",
    1e-3,
  );

  const intersected = apply(
    "INTERSECT",
    [select(firstId, secondId), ENTER],
    document,
    [firstId, secondId],
  ...
  near(
    solid3dMassProperties(soleSolid(intersected)).volume,
    25_000,
  ...
  assert.match(
    messageOf(run("UNION", [select(firstId), ENTER], document, [firstId])),
    /DOS sólidos/,
  );
}
...
// --- FILLETEDGE / CHAMFEREDGE -------------------------------------------------
{
  ...
  document = apply("EXTRUDE", [select("base"), distance(100)], document, [
    "base",
  ]);
  ...
  const chamfered = apply(
    "CHAMFEREDGE",
    [select(solidId), distance(10)],
    document,
    [solidId],
  );
  ...
  assert.equal(
    chamferedSolid.id,
    solidId,
    "CHAMFEREDGE conserva el id: `replace`, no borrar+crear",
  );
  assert.ok(
  ...
  assert.ok(
    validateBody(solid3dBody(chamferedSolid), {
      requireClosed: true,
      requirePlanarFaces: true,
    }).ok,
  ...
  assert.equal(
    last.op,
    "chamfer",
    "el árbol crece con el nodo de la operación: sigue siendo reeditable",
  );

  const filleted = apply(
    "FILLETEDGE",
    [select(solidId), distance(10)],
    document,
    [solidId],
...
{
  let document = documentWith([rectangle("base", 0, 0, 100, 100)]);
  document = apply("EXTRUDE", [select("base"), distance(40)], document, [
    "base",
  ]);
  const solidId = soleSolid(document).id;
  ...
  near(
    solid3dMassProperties(soleSolid(sliced)).volume,
    200_000,
    "media pieza tras SLICE",
    1e-3,
  );
  ...
  const total = halves.reduce(
    (sum, half) =>
      sum + (half.type === "solid3d" ? solid3dMassProperties(half).volume : 0),
    0,
  );
  ...
  const regions = sectioned.entities.filter(
    (entity) => entity.type === "region",
  );
  assert.equal(
    regions.length,
    1,
    "SECTION deja la huella del corte como REGION",
  );
  ...
  assert.equal(
    region.outer.length,
    4,
    "la sección de un prisma recto es un rectángulo",
  );
...
// --- MASSPROP (de la ola de consultas) / INTERFERE -----------------------------
{
  ...
  const mass = messageOf(
    run("MASSPROP", [select(firstId), ENTER], document, [firstId]),
  );
  assert.match(
    mass,
    /SOLID3D/,
    "MASSPROP reconoce el sólido en vez de apartarse",
  );
  assert.match(mass, /volumen/, "y publica el volumen");
  assert.match(
    mass,
    /deriva/,
    "y la discrepancia entre los DOS caminos, que es lo que permite fiarse",
  );

  const interfering = messageOf(
    run("INTERFERE", [select(firstId, secondId), ENTER], document, [
      firstId,
      secondId,
    ]),
  );
  assert.match(interfering, /INTERFIEREN/, "dos piezas solapadas interfieren");
  // El separador es la COMA porque `formatMagnitude` publica en es-MX. Esta
  // aserción esperaba `25.000` —el punto de millares español— y pasaba porque
  // el formateador usaba `es-ES`. Es el test que fijaba el defecto.
  assert.match(
    interfering,
    /25,000|25000/,
    "y el volumen común es 50 × 50 × 10",
  );

  // Dos piezas separadas NO interfieren, y eso no es un error de cálculo.
  let apart = documentWith([
    rectangle("c", 0, 0, 10, 10),
    rectangle("d", 1_000, 1_000, 10, 10),
  ]);
...
// --- IMPORT / EXPORT: la ida y vuelta completa --------------------------------
{
  ...
  document = apply("EXTRUDE", [select("base"), distance(20)], document, [
    "base",
  ]);
  ...
  for (const format of ["step", "iges"] as const) {
    const file = exportSolidEntity(solid, {
      format,
    ...
    assert.equal(
      back.format,
      format,
      "el formato se detecta por la cabecera, no se pregunta",
    );
    assert.equal(
      back.entity.nodes.length,
      1,
      "lo importado entra como UN nodo `brep`",
    );
    assert.equal(
      back.entity.nodes[0].op,
      "brep",
      "no se inventa una receta que el archivo no traía",
    );
    ...
    near(
      solid3dMassProperties(back.entity).volume,
      volume,
      `${format}: el volumen se conserva`,
      1e-6,
    );
    assert.equal(
      eulerCounts(body).characteristic,
      2,
      `${format}: χ = 2 tras la ida y vuelta`,
    );
  }

  // Y por el COMANDO, que es como lo teclea un usuario.
  const exported = messageOf(
    run("EXPORT", [select(solid.id), keyword("STEP")], document, [solid.id]),
  );
  ...
  assert.ok(
    importResult && importResult.kind === "document",
    "IMPORT escribe el sólido en el documento",
  );
  if (importResult.kind !== "document") throw new Error("tipo");
  const insert = importResult.commands.find(
    (command): command is Extract<CadEntityCommand, { type: "insert" }> =>
      command.type === "insert",
  );
  assert.ok(insert, "IMPORT inserta una entidad");
  if (insert.entity.type !== "solid3d") throw new Error("tipo");
  near(
    solid3dMassProperties(insert.entity).volume,
    volume,
    "el volumen sobrevive al comando IMPORT",
    1e-6,
  );

  // Un texto que no es ninguno de los dos formatos se rechaza diciéndolo.
  assert.match(
    messageOf(run("IMPORT", [text("esto no es un archivo")], documentWith([]))),
    /no parece/,
  );
}
...
{
  let document = documentWith([
    rectangle("base", 0, 0, 200, 120),
    rectangle("hueco", 60, 40, 40, 40),
  ]);
  document = apply("EXTRUDE", [select("base"), distance(50)], document, [
    "base",
  ]);
  const baseId = soleSolid(document).id;
  document = apply("EXTRUDE", [select("hueco"), distance(50)], document, [
    "hueco",
  ]);
  const huecoId = document.entities
    ...
    .map((entity) => entity.id)
    .find((id) => id !== baseId)!;
  document = apply("SUBTRACT", [select(baseId, huecoId), ENTER], document, [
    baseId,
    huecoId,
  ]);
  ...
  near(
    solid3dMassProperties(built).volume,
    1_120_000,
    "volumen de la pieza acabada",
    1e-3,
  );
  ...
  assert.equal(
    ...
    3,
    "el ÁRBOL sobrevive al guardado, no una malla",
  );
  assert.ok(
  ...
  clearSolidCache();
  near(
    solid3dMassProperties(restored).volume,
    1_120_000,
    "y el volumen se reconstruye al reabrir",
    1e-3,
  );
  assert.equal(
    eulerCounts(solid3dBody(restored)).genus,
    1,
    "la pieza reabierta sigue teniendo su agujero",
  );
}
