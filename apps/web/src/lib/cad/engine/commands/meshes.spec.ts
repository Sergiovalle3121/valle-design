...
{
  assert.ok(CAD_COMMAND_REGISTRY_V2.get("MESHSMOOTHLESS"), "MESHSMOOTHLESS está en el registro");
  ...
  assert.ok(
    lessResult?.kind === "message" && lessResult.text.includes("nivel minimo"),
    `MESHSMOOTHLESS informa nivel minimo: ${lessResult?.kind === "message" ? lessResult.text : ""}`,
  );
}

// --- MESHSMOOTHLESS: cancelación ---------------------------------------------
{
  const descriptor = CAD_COMMAND_REGISTRY_V2.get("MESHSMOOTHLESS")!;
  const doc = emptyDocument();
  const context = makeContext(doc);
  let step = descriptor.begin(context);
  step = descriptor.step(step.state, { kind: "cancel" }, context);
  assert.ok(
    step.result?.kind === "message" && step.result.text.includes("cancelado"),
    "MESHSMOOTHLESS se cancela limpiamente",
  );
}
