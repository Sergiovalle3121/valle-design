/** Tests de cad-intent (Fase 69). npx tsx src/lib/cad/cad-intent.spec.ts */
import {
  CAD_TOOLS,
  describeCadIntent,
  normalizeToolCall,
...
ok(
  CAD_TOOLS.length >= 6 &&
    CAD_TOOLS.every((t) => t.type === "function" && !!t.function.name),
  "CAD_TOOLS bien formado",
);
ok(
  CAD_TOOLS.some((t) => t.function.name === "placeAsset"),
  "incluye placeAsset",
);
ok(
  CAD_TOOLS.some((t) => t.function.name === "cleanupGeometry"),
  "incluye cleanupGeometry",
);

// ── setFootprint ──
{
  const r = normalizeToolCall("setFootprint", {
    footprintW: 20000,
    footprintH: 10000,
  });
  ok(
    r.ok && r.intent.kind === "setFootprint" && r.intent.footprintW === 20000,
    "setFootprint válido",
  );
}
{
...
}
{
  const r = normalizeToolCall(
    "setFootprint",
    '{"footprintW":1000,"footprintH":500,"gridSize":250}',
  );
  ok(
    r.ok && r.intent.kind === "setFootprint" && r.intent.gridSize === 250,
    "acepta args como string JSON",
  );
}
...
// ── placeAsset ──
{
  ...
  ok(
    r.ok &&
      r.intent.kind === "placeAsset" &&
      r.intent.asset.w === 1000 &&
      r.intent.asset.h === 800,
    "placeAsset con defaults de tamaño",
  );
}
{
  const r = normalizeToolCall("placeAsset", {
  ...
  ok(
    r.ok &&
      r.intent.kind === "placeAsset" &&
      r.intent.asset.kind === "robot" &&
      r.intent.asset.rotation === 90 &&
      r.intent.asset.label === "R1",
    "placeAsset normaliza kind a minúsculas",
  );
}
{
...
// ── drawWall / addDimension → DrawAction addSegment ──
{
  ...
  ok(
    r.ok && r.intent.kind === "draw" && r.intent.action.type === "addSegment",
    "drawWall → addSegment",
  );
}
{
...
// ── moveStation ──
{
  ...
  ok(
    r.ok &&
      r.intent.kind === "moveStation" &&
      r.intent.station === "EST-10" &&
      r.intent.x === 1500,
    "moveStation válido",
  );
}
...
// ── batch ──
{
  ...
  ok(
    intents.length === 2 && errors.length === 1,
    "batch separa válidos de inválidos",
  );
}
...
// ── describeCadIntent (etiquetas de la propuesta IA, ADR §215) ──
{
  ...
  ok(
    r.ok && describeCadIntent(r.intent).includes("EST-10"),
    "describe moveStation con nombre",
  );
}
{
  const r = normalizeToolCall("placeAsset", {
...
}
{
  ...
  ok(
    r.ok && r.intent.kind === "cleanupGeometry" && r.intent.tolerance === 2,
    "cleanupGeometry valido",
  );
}
{
...
}
{
  ...
  ok(
    r.ok && describeCadIntent(r.intent).includes("diff"),
    "describe cleanup comunica preview/diff",
  );
}
