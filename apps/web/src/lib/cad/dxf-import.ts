...
function mapDxfEntityToPrimitiveEnElPlano(entity: any): {
 ...
 */
function puntosQueDefinen(entity: any): unknown[] {
 ...
 *
 * Se envuelve en vez de tocar los quince puntos de retorno de la función de
 * dentro: el aviso no depende de qué primitiva salga, sólo de dónde vivía la
 * entidad, y repartirlo por cada rama invitaba a olvidarlo en la siguiente que
 * se añadiera. Los llamadores ya miran `primitive` y `warning` por separado, así
 * que devolver los dos a la vez no obliga a cambiar ninguno.
 *
 * Si la entidad ya traía un aviso, ése se respeta: no importó limpia de todas
 * formas, y sustituirlo por el de la cota escondería el motivo principal.
 */
export function mapDxfEntityToPrimitive(entity: any): {
  primitive?: CadDxfPrimitive;
  warning?: CadDxfImportWarning;
} {
  const salida = mapDxfEntityToPrimitiveEnElPlano(entity);
