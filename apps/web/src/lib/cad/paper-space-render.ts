...
export function renderEntity(
    ...
    return [
      {
        ...
        point: paper,
        text: entity.text,
        size: Math.max(1.5, effectiveHeight * scale),
  ...
  if (entity.type === "dimension") {
    ...
    commands.push({
      ...
      text: geometry.label,
      size: Math.max(1.5, ((effectiveEntity as { textHeight?: number }).textHeight ?? (effectiveEntity.arrowSize ?? 180) * 0.55) * Math.hypot(matrix.a, matrix.b)),
  ...
  if (entity.type === "mleader") {
    ...
    commands.push({
      ...
      point: point(matrix, geometry.textAnchor),
      text: entity.text,
      size: Math.max(1.5, (entity.textHeight ?? 120) * scale),
  ...
  if (entity.type === "insert") {
    ...
    Object.entries(block.attributes ?? {}).forEach(([key, definition], index) => {
      ...
      commands.push({
        ...
        text: value,
        size: Math.max(1.5, (definition.height ?? 120) * Math.hypot(nestedMatrix.a, nestedMatrix.b) * Math.hypot(context.viewportMatrix.a, context.viewportMatrix.b)),
