...
/** SHA del mismo corpus hoy, ya con esquema 9. */
...
// La subida al 9 tampoco tiene dónde escribir en este corpus: ninguna capa
...
const apariciones = serializado.split(`"schema":9`).length - 1;
assert.equal(
  apariciones,
  1,
...
// de antes. Tres saltos, porque el corpus ha sobrevivido a tres subidas.
const comoEsquema8 = serializado.replace(`"schema":9`, `"schema":8`);
assert.equal(
  sha(comoEsquema8),
  SHA_CON_ESQUEMA_8,
...
console.log(
  `OK procedencia del SHA: ${ENTIDADES} entidades, único cambio "schema":6 -> 7 -> 8 -> 9, presupuestos no recalibrados con motivo medido`,
);
