...
assert.equal(
  snap({ x: 10.2, y: 0 }, semantics, { tolerance: 1 })?.type,
  "quadrant",
);
assert.equal(
  snap({ x: 8.1, y: 6 }, semantics, { tolerance: 1 })?.type,
  "tangent",
);
assert.equal(
  snap({ x: 4.1, y: 4 }, semantics, { tolerance: 1 })?.type,
  "insertion",
);
assert.equal(
  snap({ x: 5.1, y: 5 }, semantics, { tolerance: 1 })?.type,
  "geometric-center",
);
...
console.log(
  "professional-snapping: referencias a objeto derivadas (intersección aparente incluida)",
);
