...
export function autoDimensions(
  ...
  const maxChain =
    Number(options.maxChain) > 0 ? Math.floor(Number(options.maxChain)) : 30;
  const tol =
    Number.isFinite(options.tol as number) && (options.tol as number) >= 0
      ? (options.tol as number)
      : 1;
  ...
  if (maxX - minX > tol)
    out.push({ x: minX, y: maxY + offset, x2: maxX, y2: maxY + offset });
  // Overall height — vertical run to the left of the bounding box.
  if (maxY - minY > tol)
    out.push({ x: minX - offset, y: minY, x2: minX - offset, y2: maxY });
  ...
  if (chain && boxes.length >= 2) {
    ...
    const xs = uniqSorted(
      boxes.map((b) => b.x + b.w / 2),
      tol,
    );
    const ys = uniqSorted(
      boxes.map((b) => b.y + b.h / 2),
      tol,
    );
    if (xs.length - 1 <= maxChain) {
      for (let i = 0; i + 1 < xs.length; i++)
        out.push({ x: xs[i], y: xBaseline, x2: xs[i + 1], y2: xBaseline });
    }
    if (ys.length - 1 <= maxChain) {
      for (let i = 0; i + 1 < ys.length; i++)
        out.push({ x: yBaseline, y: ys[i], x2: yBaseline, y2: ys[i + 1] });
    }
  }
