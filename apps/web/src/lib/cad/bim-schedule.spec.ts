...
const near = (actual: number | undefined, expected: number, what: string, epsilon = 1e-6) => {
  assert.ok(
...
// --- 1. un local: área a ejes y área útil, las dos, y distintas --------------
{
  ...
  ok(schedule.rooms.length === 1, `los cuatro muros cierran UN local: ${schedule.rooms.length}`);
  ...
  ok(room.wallIds.join(",") === "este,norte,oeste,sur", "y nombra los cuatro muros que lo cierran");
...
// --- 2. una T parte la planta en dos locales, sin que nadie los dibuje --------
{
  ...
  ok(schedule.rooms.length === 2, `el tabique crea DOS locales: ${schedule.rooms.length}`);
  const [first, second] = schedule.rooms;
  near(first.axisArea, 12_000_000, "el mayor, a la derecha del tabique: 3.000 × 4.000");
  near(second.axisArea, 8_000_000, "el menor, a la izquierda: 2.000 × 4.000");
  // Útil de la derecha: (3000 − 75 − 125) × (4000 − 125 − 125).
  near(first.clearArea, 10_500_000, "área útil del mayor, con el medio tabique descontado");
...
// --- 3. cantidades de muro con los huecos descontados ------------------------
{
  ...
  const schedule = buildCadBimSchedule(documentOf([...shell(), puerta, ventana]));

  ok(schedule.walls.length === 1, "los cuatro muros comparten capa y grosor: UNA fila");
  const row = schedule.walls[0];
  ok(row.count === 4, "con cuatro unidades");
  near(row.length, 18_000, "18 m de eje");
  near(row.openingArea, 900 * 2_100 + 1_200 * 1_200, "hueco descontado: puerta + ventana");
  near(row.faceArea, 18_000 * 2_400 - (900 * 2_100 + 1_200 * 1_200), "paramento neto");
  near(row.volume, row.faceArea * 250, "volumen de fábrica sobre el paramento neto");

  ok(schedule.openings.length === 2, "dos tipos de carpintería");
  ok(
    schedule.openings.map((opening) => opening.mark).join(" ") === "P-090x210 V-120x120",
    `con la marca de despacho: ${schedule.openings.map((opening) => opening.mark).join(" ")}`,
  );
  ok(schedule.openings.every((opening) => opening.count === 1), "una unidad de cada");
}
...
// --- 4. dos puertas iguales son UNA fila con dos unidades --------------------
{
  ...
  const schedule = buildCadBimSchedule(
    documentOf([...shell(), base, { ...base, id: "p2", position: 3_500, swing: "right" }]),
  );
  ok(schedule.openings.length === 1, "la mano no crea un tipo nuevo de carpintería");
...
// --- 5. la tabla cambia cuando cambia el modelo ------------------------------
{
  ...
  const stretched = executeCadEntityCommandBatch(
    ...
    [
      ...
      { type: "properties", entityId: "este", patch: { startX: 6_000, endX: 6_000 } },
  ...
  near(after.rooms[0].axisArea, 24_000_000, "después, 24 m²: la tabla siguió al modelo");
...
// --- 6. un hueco huérfano no se cuenta, y se DICE ----------------------------
{
  ...
  near(schedule.walls[0].openingArea, 0, "…ni descuenta paramento de ningún muro…");
  ok(
