 ...
 *
 * Correr tests:  npx tsx src/lib/cad/cad-vision.spec.ts
 */
...
export function normalizeVision(
  ...
  const mapPt = (nx: number, ny: number): Point => ({
    x: clamp01(nx) * W,
    y: (1 - clamp01(ny)) * H,
  });
  ...
  rawWalls.forEach((w, i) => {
    ...
    if (x1 === null || y1 === null || x2 === null || y2 === null) {
      errors.push(`muro ${i}: coords inválidas`);
      return;
    }
    const a = mapPt(x1, y1);
    const b = mapPt(x2, y2);
    if (Math.hypot(b.x - a.x, b.y - a.y) < 1e-6) {
      errors.push(`muro ${i}: longitud cero`);
      return;
    }
    walls.push({ a, b });
  });
  ...
  rawZones.forEach((z, i) => {
    ...
    if (points.length < 3) {
      errors.push(`zona ${i}: necesita ≥3 puntos`);
      return;
    }
    zones.push({
      ...(typeof o.name === "string" ? { name: o.name.slice(0, 48) } : {}),
      points,
    });
  });

  const unitHint =
    data.unitHint === "mm" || data.unitHint === "m" ? data.unitHint : undefined;
