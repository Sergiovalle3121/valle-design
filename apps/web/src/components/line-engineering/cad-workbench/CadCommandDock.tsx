  ...
  return (
    <div className="border-b border-indigo-400/20 bg-indigo-400/[0.06] p-3">
      ...
      <form className="mt-2 flex gap-1.5" onSubmit={(event) => { event.preventDefault(); onSubmit(); }}>
        ...
        />
        <button type="submit" className="rounded-lg bg-indigo-500 px-2.5 py-1.5 type-caption font-semibold text-white hover:bg-indigo-400">Preview</button>
        <button type="button" onClick={() => onRequestAi('intent')} disabled={aiBusy !== null} title="Interpreta la instrucción con el motor de IA (CIDE)" className="rounded-lg bg-violet-600 px-2.5 py-1.5 type-caption font-semibold text-white hover:bg-violet-500 disabled:opacity-50">{aiBusy === 'intent' ? '…' : 'IA'}</button>
      </form>
      <button type="button" onClick={() => onRequestAi('optimize')} disabled={aiBusy !== null} className="mt-1.5 w-full rounded-lg border border-violet-400/30 bg-violet-400/[0.08] px-2 py-1.5 type-micro font-semibold text-violet-200 hover:bg-violet-400/[0.14] disabled:opacity-50">
      ...
      {aiProposal && (
        <div className="mt-2 rounded-xl border border-violet-400/25 bg-gray-950/70 p-2">
          <div className="type-micro font-semibold text-violet-200">Propuesta IA · {aiProposal.source === 'intent' ? 'instrucción' : 'optimización'}</div>
          {aiProposal.message && <div className="mt-1 type-micro text-amber-200">{aiProposal.message}</div>}
          {aiProposal.intents.slice(0, 6).map((intent, index) => (
            <div key={`ai-${index}`} className="mt-1 rounded-md bg-white/[0.04] px-1.5 py-1 type-micro text-gray-300">{aiProposal.descriptions?.[index] ?? describeCadIntent(intent)}</div>
          ))}
          {aiProposal.intents.length > 6 && <div className="mt-1 type-micro text-gray-500">…y {aiProposal.intents.length - 6} acción(es) más.</div>}
          {aiProposal.errors.slice(0, 2).map((error) => <div key={error} className="mt-1 type-micro text-rose-300">Descartada: {error}</div>)}
          <div className="mt-2 flex gap-1.5">
            {aiProposal.intents.length > 0 && <button type="button" onClick={onApplyAi} className="rounded-lg bg-emerald-700 px-2 py-1 type-micro font-semibold text-white hover:bg-emerald-600">Aplicar {aiProposal.intents.length}</button>}
            <button type="button" onClick={onDiscardAi} className="rounded-lg border border-white/10 px-2 py-1 type-micro text-gray-300 hover:bg-white/10">Descartar</button>
          </div>
        </div>
      )}
      ...
      {preview && (
        <div className="mt-2 rounded-xl border border-white/10 bg-gray-950/70 p-2">
          <div className="type-micro font-semibold text-white">{preview.preview.summary}</div>
          <div className="mt-1 type-micro text-gray-500 dark:text-gray-400">{preview.preview.affectedObjectIds.length} objeto(s) · {preview.preview.operations.length} operación(es)</div>
          {preview.preview.operations.slice(0, 3).map((operation, index) => (
            <div key={`${operation.type}-${index}`} className="mt-1 rounded-md bg-white/[0.04] px-1.5 py-1 type-micro text-gray-300">
          ...
          {preview.preview.issues.slice(0, 2).map((issue) => <div key={`${issue.code}-${issue.message}`} className={`mt-1 type-micro ${issue.level === 'error' ? 'text-rose-300' : issue.level === 'warning' ? 'text-amber-300' : 'text-indigo-200'}`}>{issue.message}</div>)}
          <div className="mt-2 flex gap-1.5">
            <button type="button" onClick={onApplyPreview} className="rounded-lg bg-emerald-700 px-2 py-1 type-micro font-semibold text-white hover:bg-emerald-600">Aplicar</button>
            <button type="button" onClick={onDismissPreview} className="rounded-lg border border-white/10 px-2 py-1 type-micro text-gray-300 hover:bg-white/10">Cancelar</button>
            <button type="button" onClick={onDismissPreview} className="rounded-lg border border-white/10 px-2 py-1 type-micro text-gray-300 hover:bg-white/10">Editar</button>
          </div>
        </div>
      )}
      ...
      {suggestions.length > 0 && (
        <div className="mt-2 space-y-1.5">
          <div className="flex items-center justify-between type-micro uppercase tracking-wide text-gray-500"><span>Sugerencias</span><span>{selectionCount} sel</span></div>
          {suggestions.map((suggestion) => (
            <button type="button" key={suggestion.id} onClick={() => onSuggestion(suggestion)} title={suggestion.example} className={`w-full rounded-lg border px-2 py-1.5 text-left hover:bg-white/[0.08] ${suggestion.ready ? 'border-indigo-400/20 bg-indigo-400/[0.05]' : 'border-amber-400/20 bg-amber-400/[0.05]'}`}>
              <span className="flex items-center justify-between gap-2"><span className="truncate type-micro font-semibold text-gray-100">{suggestion.label}</span><span className={`shrink-0 type-micro uppercase tracking-wide ${suggestion.ready ? 'text-indigo-200' : 'text-amber-200'}`}>{suggestion.ready ? 'Preview' : 'Pendiente'}</span></span>
              <span className="mt-0.5 block truncate type-micro text-gray-400">{suggestion.example}</span>
              <span className={`mt-0.5 block truncate type-micro ${suggestion.ready ? 'text-indigo-200/70' : 'text-amber-200/80'}`}>{suggestion.reason}</span>
            </button>
          ))}
        </div>
      )}
      ...
      {log.length > 0 && (
        <div className="mt-2 space-y-1">
          <div className="flex items-center justify-between type-micro uppercase tracking-wide text-gray-500"><span>Historial</span><span>{log.length}</span></div>
          <div className="flex gap-1.5">
            <button type="button" onClick={onUndo} disabled={!canUndo} className="flex-1 rounded-lg border border-white/10 px-2 py-1 type-micro text-gray-300 disabled:opacity-40 hover:bg-white/10">Deshacer cmd</button>
            <button type="button" onClick={onRedo} disabled={!canRedo} className="flex-1 rounded-lg border border-white/10 px-2 py-1 type-micro text-gray-300 disabled:opacity-40 hover:bg-white/10">Rehacer cmd</button>
          </div>
          {log.slice(0, 3).map((item) => <div key={item.id} className="rounded-lg bg-white/[0.04] px-2 py-1 type-micro text-gray-300"><span className={item.status === 'failed' ? 'text-rose-300' : item.status === 'applied' ? 'text-emerald-300' : 'text-indigo-200'}>{item.status}</span> · {item.label}{item.durationMs !== undefined ? ` · ${item.durationMs} ms` : ''}{item.affectedObjectIds?.length ? ` · ${item.affectedObjectIds.length} obj` : ''}</div>)}
        </div>
      )}
    </div>
  );
