    ...
    <div className="border-b border-cyan-400/20 bg-cyan-400/[0.06] p-3">
      ...
      {preview && (
        <div className="mt-2 rounded-xl border border-white/10 bg-gray-950/70 p-2">
          ...
          {preview.preview.operations.slice(0, 3).map((operation, index) => (
            <div key={`${operation.type}-${index}`} className="mt-1 rounded-md bg-white/[0.04] px-1.5 py-1 text-[10.5px] text-gray-300">
              {operation.type === 'report' ? (
                <div>
                  <div className="font-semibold text-cyan-100">{operation.title}</div>
          ...
          {preview.preview.issues.slice(0, 2).map((issue) => <div key={`${issue.code}-${issue.message}`} className={`mt-1 text-[10.5px] ${issue.level === 'error' ? 'text-rose-300' : issue.level === 'warning' ? 'text-amber-300' : 'text-cyan-200'}`}>{issue.message}</div>)}
          <div className="mt-2 flex gap-1.5">
      ...
      {suggestions.length > 0 && (
        <div className="mt-2 space-y-1.5">
          ...
          {suggestions.map((suggestion) => (
            <button type="button" key={suggestion.id} onClick={() => onSuggestion(suggestion)} title={suggestion.example} className={`w-full rounded-lg border px-2 py-1.5 text-left hover:bg-white/[0.08] ${suggestion.ready ? 'border-cyan-400/20 bg-cyan-400/[0.05]' : 'border-amber-400/20 bg-amber-400/[0.05]'}`}>
              <span className="flex items-center justify-between gap-2"><span className="truncate text-[11px] font-semibold text-gray-100">{suggestion.label}</span><span className={`shrink-0 text-[9.5px] uppercase tracking-wide ${suggestion.ready ? 'text-cyan-200' : 'text-amber-200'}`}>{suggestion.ready ? 'Preview' : 'Pendiente'}</span></span>
              <span className="mt-0.5 block truncate text-[10.5px] text-gray-400">{suggestion.example}</span>
              <span className={`mt-0.5 block truncate text-[10px] ${suggestion.ready ? 'text-cyan-200/70' : 'text-amber-200/80'}`}>{suggestion.reason}</span>
            </button>
          ))}
        </div>
      )}
      ...
      {log.length > 0 && (
        <div className="mt-2 space-y-1">
          ...
          {log.slice(0, 3).map((item) => <div key={item.id} className="rounded-lg bg-white/[0.04] px-2 py-1 text-[10.5px] text-gray-300"><span className={item.status === 'failed' ? 'text-rose-300' : item.status === 'applied' ? 'text-emerald-300' : 'text-cyan-200'}>{item.status}</span> · {item.label}{item.durationMs !== undefined ? ` · ${item.durationMs} ms` : ''}{item.affectedObjectIds?.length ? ` · ${item.affectedObjectIds.length} obj` : ''}</div>)}
        </div>
      )}
    </div>
  );
