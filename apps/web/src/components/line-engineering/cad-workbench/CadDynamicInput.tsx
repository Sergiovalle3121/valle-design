...
const fieldLabels: Record<Field, string> = {
  x: 'X', y: 'Y', distance: 'Distancia', angle: 'Ángulo',
  radius: 'Radio', diameter: 'Diámetro', offset: 'Offset',
};
...
export function CadDynamicInput({
  ...
  const [pointMode, setPointMode] = useState<CadDynamicInputMode>(anchor ? 'relative' : 'absolute');
  ...
  const mode: CadDynamicInputMode = kind === 'point' ? pointMode : kind === 'radius' ? radiusMode : 'offset';
  const fields: Field[] = mode === 'absolute' || mode === 'relative'
    ? ['x', 'y']
    : mode === 'polar'
      ? ['distance', 'angle']
      : [mode];
        ...
        <label key={field} className="inline-flex items-center gap-1 rounded-md border border-white/10 bg-gray-950/80 px-1.5 py-0.5">
          ...
          <input
            ...
            onKeyDown={(event) => {
              ...
              if (!next) return;
              event.preventDefault();
              event.currentTarget.form?.querySelector<HTMLInputElement>(`[data-testid="cad-dynamic-field-${next}"]`)?.focus();
            }}
          ...
          <button
            ...
            className={locked[field] ? 'text-amber-300' : 'text-gray-600 hover:text-gray-300'}
          >
            {locked[field] ? '●' : '○'}
          </button>
        </label>
