  ...
  return (
    <div data-testid="cad-mleader-palette" className={docked ? 'w-full p-3 type-micro' : 'absolute right-0 top-full z-50 mt-1.5 w-80 rounded-xl border border-indigo-400/20 bg-gray-950 p-3 type-micro shadow-2xl'}>
      ...
      <div className="mt-2 grid grid-cols-2 gap-2">
        <label className="text-gray-400">Tipo<select value={draft.contentType} onChange={(event) => setDraft((current) => ({ ...current, contentType: event.target.value as CadMLeaderDraft['contentType'] }))} className="mt-1 w-full rounded border border-white/10 bg-gray-900 px-2 py-1 text-white"><option value="mtext">MTEXT</option><option value="text">Text</option></select></label>
        <label className="text-gray-400">Estilo<input list="cad-mleader-styles" value={draft.style} onChange={(event) => setDraft((current) => ({ ...current, style: event.target.value }))} className="mt-1 w-full rounded border border-white/10 bg-black/30 px-2 py-1 text-white" /><datalist id="cad-mleader-styles">{styles.map((style) => <option key={style} value={style} />)}</datalist></label>
        <label className="text-gray-400">Arrow<select data-testid="cad-mleader-arrow" value={draft.arrowhead} onChange={(event) => setDraft((current) => ({ ...current, arrowhead: event.target.value as CadMLeaderDraft['arrowhead'] }))} className="mt-1 w-full rounded border border-white/10 bg-gray-900 px-2 py-1 text-white"><option value="closed-filled">Closed filled</option><option value="open">Open</option><option value="architectural-tick">Architectural tick</option><option value="dot">Dot</option><option value="none">None</option></select></label>
        <label className="text-gray-400">Arrow size<input type="number" min="0.001" value={draft.arrowSize} onChange={(event) => setDraft((current) => ({ ...current, arrowSize: Math.max(0.001, Number(event.target.value) || defaultSize) }))} className="mt-1 w-full rounded border border-white/10 bg-black/30 px-2 py-1 text-white" /></label>
        <label className="text-gray-400">Dogleg<input data-testid="cad-mleader-dogleg" type="number" min="0.001" value={draft.doglegLength} onChange={(event) => setDraft((current) => ({ ...current, doglegLength: Math.max(0.001, Number(event.target.value) || defaultSize * 4) }))} className="mt-1 w-full rounded border border-white/10 bg-black/30 px-2 py-1 text-white" /></label>
        <label className="text-gray-400">Alineación<select value={draft.textAlignment} onChange={(event) => setDraft((current) => ({ ...current, textAlignment: event.target.value as CadMLeaderDraft['textAlignment'] }))} className="mt-1 w-full rounded border border-white/10 bg-gray-900 px-2 py-1 text-white">{['left', 'center', 'right', 'justify'].map((value) => <option key={value}>{value}</option>)}</select></label>
