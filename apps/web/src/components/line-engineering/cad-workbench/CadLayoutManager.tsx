  ...
  return (
    <section data-testid="cad-layout-manager" className="col-span-full mt-2 space-y-3 border-t border-white/10 pt-3">
      <div className="flex flex-wrap items-center gap-2">
        <span className="mr-auto type-micro font-semibold uppercase tracking-[0.15em] text-indigo-200">Viewports · {space.viewports?.length ?? 0}</span>
        <button data-testid="cad-viewport-add" onClick={props.onAdd} className="inline-flex items-center gap-1 rounded-lg bg-indigo-600 px-2 py-1 type-micro font-semibold text-white hover:bg-indigo-500"><Plus className="h-3 w-3" />Viewport</button>
        <button disabled={!active} onClick={() => active && props.onDuplicate(active.id)} className="inline-flex items-center gap-1 rounded-lg border border-white/10 px-2 py-1 type-micro disabled:opacity-35"><Copy className="h-3 w-3" />Duplicar</button>
        <button disabled={!active} onClick={() => active && props.onDelete(active.id)} className="inline-flex items-center gap-1 rounded-lg border border-rose-300/20 px-2 py-1 type-micro text-rose-200 disabled:opacity-35"><Trash2 className="h-3 w-3" />Eliminar</button>
      </div>
      ...
      <div className="grid gap-3 lg:grid-cols-[1.1fr_0.9fr]">
        <div>
          <div ref={paperRef} data-testid="cad-paper-preview" className="relative mx-auto w-full overflow-hidden rounded-sm bg-white shadow-xl" style={{ aspectRatio: `${space.page.width}/${space.page.height}` }} onPointerMove={moveDrag} onPointerUp={endDrag} onPointerCancel={endDrag}>
            ...
            <div className="pointer-events-none absolute inset-x-[2%] bottom-[2%] h-[10%] border border-slate-600 type-micro text-slate-600">TITLE BLOCK</div>
          </div>
          <div className="mt-2 flex flex-wrap gap-1.5">
            {(space.viewports ?? []).map((viewport) => <button key={viewport.id} onClick={() => props.onActivate(viewport.id)} className={`rounded-full border px-2 py-0.5 type-micro ${active?.id === viewport.id ? 'border-indigo-300/50 bg-indigo-400/15 text-indigo-100' : 'border-white/10 text-gray-400'}`}>{viewport.name ?? viewport.id}</button>)}
          </div>
        </div>

        {active ? <div className="space-y-2 rounded-xl border border-white/10 bg-gray-950/45 p-2.5">
          <div className="flex items-center gap-2"><input data-testid="cad-viewport-name" value={active.name ?? ''} onChange={(event) => props.onChange(active.id, (viewport) => ({ ...viewport, name: event.target.value.slice(0, 80) }), 'Renombrar viewport')} className="min-w-0 flex-1 rounded-lg border border-white/10 bg-gray-950/70 px-2 py-1.5 type-micro text-white" /><button data-testid="cad-viewport-lock" onClick={() => props.onChange(active.id, (viewport) => ({ ...viewport, locked: !viewport.locked }), active.locked ? 'Desbloquear viewport' : 'Bloquear viewport')} className={`rounded-lg p-1.5 ${active.locked ? 'bg-emerald-400/10 text-emerald-200' : 'bg-amber-400/10 text-amber-200'}`}>{active.locked ? <Lock className="h-3.5 w-3.5" /> : <LockOpen className="h-3.5 w-3.5" />}</button></div>
          <div className="grid grid-cols-4 gap-1.5">{(['x', 'y', 'width', 'height'] as const).map((field) => <label key={`paper-${field}`} className="type-micro text-gray-500">Paper {field}<input data-testid={`cad-viewport-paper-${field}`} type="number" value={fieldValue(active.paperBounds[field])} disabled={active.locked} onChange={(event) => updateNumber('paperBounds', field, Number(event.target.value))} className="mt-0.5 w-full rounded border border-white/10 bg-gray-950/70 px-1 py-1 type-micro text-white disabled:opacity-40" /></label>)}</div>
          <div className="grid grid-cols-4 gap-1.5">{(['x', 'y', 'width', 'height'] as const).map((field) => <label key={`model-${field}`} className="type-micro text-gray-500">Model {field}<input data-testid={`cad-viewport-model-${field}`} type="number" value={fieldValue(active.modelBounds[field])} disabled={active.locked} onChange={(event) => updateNumber('modelBounds', field, Number(event.target.value))} className="mt-0.5 w-full rounded border border-white/10 bg-gray-950/70 px-1 py-1 type-micro text-white disabled:opacity-40" /></label>)}</div>
          <div className="grid grid-cols-3 gap-1.5">
            <label className="type-micro text-gray-500">Standard scale<select data-testid="cad-viewport-scale" value={CAD_SHEET_SCALES.includes(active.scale as typeof CAD_SHEET_SCALES[number]) ? active.scale : 'custom'} disabled={active.locked} onChange={(event) => event.target.value !== 'custom' && props.onChange(active.id, (viewport) => ({ ...viewport, scale: Number(event.target.value) }), 'Escala estándar')} className="mt-0.5 w-full rounded border border-white/10 bg-gray-950/70 px-1 py-1 type-micro text-white disabled:opacity-40"><option value="custom">Custom</option>{CAD_SHEET_SCALES.map((scale) => <option key={scale} value={scale}>1:{scale}</option>)}</select></label>
            <label className="type-micro text-gray-500">Custom scale<input data-testid="cad-viewport-custom-scale" type="number" min="0.001" value={active.scale} disabled={active.locked} onChange={(event) => Number(event.target.value) > 0 && props.onChange(active.id, (viewport) => ({ ...viewport, scale: Number(event.target.value) }), 'Escala personalizada')} className="mt-0.5 w-full rounded border border-white/10 bg-gray-950/70 px-1 py-1 type-micro text-white disabled:opacity-40" /></label>
            <label className="type-micro text-gray-500">Annotation scale<input data-testid="cad-viewport-annotation-scale" type="number" min="0.001" value={active.annotationScale ?? active.scale} onChange={(event) => Number(event.target.value) > 0 && props.onChange(active.id, (viewport) => ({ ...viewport, annotationScale: Number(event.target.value) }), 'Escala anotativa')} className="mt-0.5 w-full rounded border border-white/10 bg-gray-950/70 px-1 py-1 type-micro text-white" /></label>
          </div>
          <label className="block type-micro text-gray-500">Named view<input data-testid="cad-viewport-named-view" value={active.namedView ?? ''} onChange={(event) => props.onChange(active.id, (viewport) => ({ ...viewport, namedView: event.target.value.slice(0, 80) }), 'Vista nombrada')} className="mt-0.5 w-full rounded border border-white/10 bg-gray-950/70 px-2 py-1 type-micro text-white" /></label>
          <div className="max-h-36 space-y-1 overflow-y-auto border-t border-white/10 pt-2">{props.layers.map((layer) => {
            ...
            return <div key={layer.id} className="grid grid-cols-[1fr_auto_auto_auto] items-center gap-1.5 rounded bg-white/[0.03] px-1.5 py-1 type-micro"><label className="flex min-w-0 items-center gap-1.5"><input data-testid={`cad-viewport-layer-${layer.id}`} type="checkbox" checked={visible} onChange={(event) => props.onLayerVisibility(active.id, layer.id, event.target.checked)} /><span className="truncate">{layer.name}</span></label><input aria-label={`Override color ${layer.name}`} type="color" value={override?.color ?? layer.color} onChange={(event) => props.onLayerOverride(active.id, layer.id, { ...override, color: event.target.value })} className="h-5 w-6 bg-transparent" /><input aria-label={`Override lineweight ${layer.name}`} type="number" step="0.05" min="0" value={override?.lineweight ?? ''} placeholder="LW" onChange={(event) => props.onLayerOverride(active.id, layer.id, { ...override, lineweight: event.target.value ? Number(event.target.value) : undefined })} className="w-11 rounded border border-white/10 bg-gray-950 px-1 py-0.5 type-micro" /><button onClick={() => props.onLayerOverride(active.id, layer.id, null)} className="text-gray-500 hover:text-white">Reset</button></div>;
          })}</div>
        </div> : <div className="grid place-items-center rounded-xl border border-dashed border-white/10 type-micro text-gray-500">Add a viewport to begin.</div>}
      </div>

      <div className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/[0.03] p-2">
        <button data-testid="cad-layout-preview-build" onClick={props.onRequestPreview} className="inline-flex items-center gap-1 rounded-lg bg-white px-2.5 py-1.5 type-micro font-semibold text-gray-900"><Eye className="h-3.5 w-3.5" />Exact print preview</button>
        <span className={`type-micro ${props.preflight.some((issue) => issue.severity === 'error') ? 'text-rose-200' : props.preflight.length ? 'text-amber-200' : 'text-emerald-200'}`}>{props.preflight.length ? `${props.preflight.length} preflight issue(s)` : 'Preflight ready'}</span>
        <span className="ml-auto type-micro text-gray-500">Drag unlocked viewports; use the lower-right grip to resize.</span>
      </div>
      {props.preflight.length > 0 && <div data-testid="cad-layout-preflight" className="grid gap-1 md:grid-cols-2">{props.preflight.map((issue, index) => <div key={`${issue.code}:${issue.viewportId ?? index}`} className={`rounded-lg border px-2 py-1 type-micro ${issue.severity === 'error' ? 'border-rose-300/20 bg-rose-400/[0.07] text-rose-100' : 'border-amber-300/20 bg-amber-400/[0.07] text-amber-100'}`}>{issue.code}: {issue.detail}</div>)}</div>}
