 ...
 * en milímetros. Correr tests:  npx tsx src/components/cad/plot/plot-scale.spec.ts
 */
import { Point } from "@/lib/cad/precision-input";
...
export const NICE_SCALES = [
  10, 20, 25, 50, 75, 100, 150, 200, 250, 500, 750, 1000, 1500, 2000, 5000,
];
...
export function fitScale(
  ...
  const offsetYmm =
    margin + titleBlockH + Math.max(0, (availH - drawingHmm) / 2);
...
export function worldToPaper(
  p: Point,
  footprint: { footprintH: number; unit: string },
  layout: PlotLayout,
): Point {
...
export function scaleBar(
  ...
  return {
    intervalReal: nice,
    intervalMm: niceRealMm / layout.scale,
    divisions,
    unit,
  };
}
