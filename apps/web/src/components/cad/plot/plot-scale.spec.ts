/** Tests de plot-scale (Fase 70). npx tsx src/components/cad/plot/plot-scale.spec.ts */
import {
  fitScale,
  worldToPaper,
  scaleBar,
...
// ── fitScale: 20 m × 10 m en A3 ──
{
  ...
  ok(
    L.scale === 50,
    `1:50 es la escala más detallada que cabe (got 1:${L.scale})`,
  );
  ok(
    near(L.drawingWmm, 20000 / 50) && near(L.drawingHmm, 10000 / 50),
    "tamaño de dibujo correcto",
  );
}
...
{
  const L = fitScale(
    { footprintW: 5_000_000, footprintH: 5_000_000, unit: "mm" },
    "A4",
  );
  ok(
    L.scale === 5000,
    "footprint gigantesco → 1:5000 (la más grande disponible)",
  );
}

// ── fitScale: centra dentro del área útil ──
{
  const L = fitScale({ footprintW: 1000, footprintH: 1000, unit: "mm" }, "A4", {
    margin: 10,
  });
...
// ── worldToPaper: origen del footprint va al pie del dibujo (flip Y) ──
{
  ...
  ok(
    near(origin.y, L.offsetYmm + L.drawingHmm),
    "y del origen al pie del dibujo (flip)",
  );
...
// ── scaleBar ──
{
  ...
  ok(
    bar.intervalReal === 2 && bar.unit === "m",
    `escalímetro 2 m por división (got ${bar.intervalReal})`,
  );
  ok(
    near(bar.intervalMm, (2 * 1000) / 50),
    "longitud en papel de la división correcta",
  );
}

// ── tamaños de papel ──
ok(
  PAPER_SIZES.A4.w === 297 && PAPER_SIZES.A0.w === 1189,
  "tamaños A4/A0 correctos (mm, horizontal)",
);
