    ...
    () =>
      new Map(
        sheet.viewports.map((viewport) => [
          viewport.id,
          `cad-clip-${viewport.id.replace(/[^a-z0-9_-]/gi, "-")}`,
        ]),
      ),
    [sheet.viewports],
  );
  return (
    ...
    >
      <svg
        ...
        className="mx-auto block max-h-[460px] w-full bg-white shadow-xl"
        aria-label={`Exact print preview ${sheet.name}`}
      >
        <defs>
          {sheet.viewports.map((viewport) => (
        ...
        <rect
          x="6"
          y="6"
          width={Math.max(1, sheet.width - 12)}
          height={Math.max(1, sheet.height - 12)}
          fill="none"
          stroke="#111827"
          strokeWidth="0.45"
        />
        {sheet.viewports.map((viewport) => (
          <g key={viewport.id}>
            ...
            />
            <text
              x={viewport.clip.x + 1.5}
              y={viewport.clip.y + 4}
              fill="#0f172a"
              fontSize="3"
            >
              {viewport.name} · 1:{viewport.scale}
            </text>
          </g>
        ))}
        <rect
        ...
        <text
          x="9"
          y={Math.max(10, sheet.height - 29)}
          fill="#0f172a"
          fontSize="4"
        >
          {sheet.titleBlock.PROJECT ?? sheet.name}
        </text>
        <text
          x="9"
          y={Math.max(14, sheet.height - 22)}
          fill="#0f172a"
          fontSize="3"
        >
          {sheet.titleBlock.DRAWING_NO ?? ""} ·{" "}
          {sheet.titleBlock.REVISION ?? ""}
        </text>
      </svg>
    </div>
  );
...
export function CadLayoutManager(props: CadLayoutManagerProps) {
  const { space } = props;
  const active =
    (space.viewports ?? []).find(
      (viewport) => viewport.id === props.activeViewportId,
    ) ??
    space.viewports?.[0] ??
    null;
  ...
  const [draftBounds, setDraftBounds] = useState<
    CadPaperViewport["paperBounds"] | null
  >(null);
  const beginDrag = (
  ...
  const moveDrag = (event: React.PointerEvent) => {
    ...
    if (!drag) return;
    const dx =
      ((event.clientX - drag.startX) / Math.max(1, drag.host.width)) *
      space.page.width;
    const dy =
      ((event.clientY - drag.startY) / Math.max(1, drag.host.height)) *
      space.page.height;
    const next =
      drag.mode === "move"
        ? {
            ...drag.viewport.paperBounds,
            x: drag.viewport.paperBounds.x + dx,
            y: drag.viewport.paperBounds.y + dy,
          }
        : {
            ...drag.viewport.paperBounds,
            width: drag.viewport.paperBounds.width + dx,
            height: drag.viewport.paperBounds.height + dy,
          };
  ...
  const endDrag = () => {
    ...
    if (drag && finalBounds)
      props.onChange(
        drag.viewport.id,
        (viewport) => ({ ...viewport, paperBounds: finalBounds }),
        drag.mode === "move" ? "Mover viewport" : "Redimensionar viewport",
      );
    dragRef.current = null;
    draftBoundsRef.current = null;
    setDraftBounds(null);
  };
  const shownBounds = (viewport: CadPaperViewport) =>
    active?.id === viewport.id && draftBounds
      ? draftBounds
      : viewport.paperBounds;
  const updateNumber = (
    section: "paperBounds" | "modelBounds",
    ...
    >
      ...
      <div className="grid gap-3 lg:grid-cols-[1.1fr_0.9fr]">
        <div>
          <div
            ref={paperRef}
            data-testid="cad-paper-preview"
            className="relative mx-auto w-full overflow-hidden rounded-sm bg-white shadow-xl"
            style={{ aspectRatio: `${space.page.width}/${space.page.height}` }}
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    width: `${(bounds.width / space.page.width) * 100}%`,
                    height: `${(bounds.height / space.page.height) * 100}%`,
                  }}
                >
                  <span className="absolute left-1 top-1 rounded bg-white/80 px-1 py-0.5">
                    {viewport.name ?? viewport.id} · 1:{viewport.scale}
                    {viewport.locked ? " · LOCK" : ""}
                  </span>
                  {!viewport.locked && (
                    <span
                      data-testid={`cad-viewport-resize-${viewport.id}`}
                      onPointerDown={(event) =>
                        beginDrag(event, viewport, "resize")
                      }
                      className="absolute bottom-0 right-0 h-3 w-3 cursor-nwse-resize border-l border-t border-indigo-700 bg-indigo-400"
                    />
                  )}
                </button>
              );
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                onClick={() =>
                  props.onChange(
                    active.id,
                    (viewport) => ({ ...viewport, locked: !viewport.locked }),
                    active.locked
              ...
              {(["x", "y", "width", "height"] as const).map((field) => (
                ...
                >
                  ...
                  <input
                    ...
                    type="number"
                    value={fieldValue(active.paperBounds[field])}
              ...
              {(["x", "y", "width", "height"] as const).map((field) => (
                ...
                >
                  ...
                  <input
                    ...
                    type="number"
                    value={fieldValue(active.modelBounds[field])}
                    disabled={active.locked}
                    onChange={(event) =>
                      updateNumber(
                        "modelBounds",
            ...
            <div className="grid grid-cols-3 gap-1.5">
              <label className="type-micro text-muted-foreground">
                ...
                <select
                  ...
                  value={
                    CAD_SHEET_SCALES.includes(
                      active.scale as (typeof CAD_SHEET_SCALES)[number],
                    )
                      ? active.scale
                      : "custom"
                  }
              ...
              <label className="type-micro text-muted-foreground">
                ...
                <input
                  ...
                  type="number"
                  min="0.001"
                  value={active.annotationScale ?? active.scale}
                  onChange={(event) =>
                    Number(event.target.value) > 0 &&
                    props.onChange(
                      active.id,
                      (viewport) => ({
                        ...viewport,
                        annotationScale: Number(event.target.value),
                      }),
                      "Escala anotativa",
                    )
                  }
              ...
              <input
                ...
                onChange={(event) =>
                  props.onChange(
                    ...
                    (viewport) => ({
                      ...viewport,
                      namedView: event.target.value.slice(0, 80),
                    }),
                    "Vista nombrada",
                  )
                }
                ...
                return (
                  ...
                  >
                    <label className="flex min-w-0 items-center gap-1.5">
                      <input
                        data-testid={`cad-viewport-layer-${layer.id}`}
                        type="checkbox"
                        checked={visible}
                        onChange={(event) =>
                          props.onLayerVisibility(
                            active.id,
                            layer.id,
                    ...
                    <input
                      aria-label={`Override color ${layer.name}`}
                      type="color"
                      value={override?.color ?? layer.color}
                      onChange={(event) =>
                        props.onLayerOverride(active.id, layer.id, {
                          ...override,
                    ...
                    />
                    <input
                      aria-label={`Override lineweight ${layer.name}`}
                      type="number"
                      step="0.05"
                      min="0"
      ...
      {props.preflight.length > 0 && (
        <div
          data-testid="cad-layout-preflight"
