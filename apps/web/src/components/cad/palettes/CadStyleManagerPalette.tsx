...
function Field({
  ...
  const label = (
    <span className="text-[10px] text-gray-500">
      {field.label}
      {inherited && (
        <span className="ml-1 text-[9px] text-gray-600" title="Valor de fábrica">
          (por defecto)
        </span>
      )}
    </span>
    ...
    return (
      <label className="flex items-center justify-between gap-2 rounded-md border border-white/10 bg-gray-950/60 px-2 py-1.5">
        ...
        <input
          ...
          disabled={readOnly}
          onChange={(event) => onEdit(row.name, field.key, event.target.checked)}
...
export const CadStyleManagerPalette = React.memo(function CadStyleManagerPalette(
  props: CadStyleManagerPaletteProps,
) {
  ...
  return (
    ...
    >
      ...
      <p className="mt-3 text-[9.5px] text-gray-500">
        Renombrar no se ofrece: las entidades referencian su estilo POR NOMBRE, y
        cambiarlo sin reasignarlas las dejaría huérfanas. Crea el nuevo, mueve
        las entidades desde la paleta de propiedades y borra el viejo.
      </p>
    </div>
  );
});
