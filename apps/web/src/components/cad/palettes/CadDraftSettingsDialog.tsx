...
function Toggle({
  ...
  return (
    ...
    >
      <span>
        {label}
        {hint && <span className="ml-1.5 text-[9.5px] text-gray-500">{hint}</span>}
      </span>
...
export const CadDraftSettingsDialog = React.memo(function CadDraftSettingsDialog({
  settings,
  modes,
  polarIncrements,
  grid,
  ...
  const activeModes = modes.filter((mode) => settings.osnapModes[mode as never]).length;
  ...
  return (
    ...
    >
      <div className="mb-3 flex items-center justify-between gap-3">
        <div>
          <div className="text-[13px] font-semibold">DSETTINGS · Ayudas al dibujo</div>
          <div className="text-[10.5px] text-gray-500">
            Captura a objetos, rastreo y rejilla. F3 / F8 / F10 / F11 conmutan lo
            ...
            {modes.map((mode) => (
              ...
              >
                ...
                <input
                  ...
                  onChange={(event) => onToggleMode(mode, event.target.checked)}
