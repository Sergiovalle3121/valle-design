...
// Los 6 arquetipos de mobiliario/arquitectura (escalera…lavabo) viven en el
// archivo hermano por la misma razón: empujaron a este por encima del
// presupuesto de check:monolith-budget.mjs. Cuerpos de switch sin más cambio.
import {
  buildStairsArchetype,
  buildRailingArchetype,
  buildSofaArchetype,
  buildBedArchetype,
  buildToiletArchetype,
  buildSinkArchetype,
} from "./asset-archetypes-architectural";
...
export function buildCadAssetArchetype(
  ...
  switch (archetype) {
    ...
    // Cuerpos movidos a asset-archetypes-architectural.ts (presupuesto de
    // check:monolith-budget.mjs); mismo comportamiento, mismos parámetros
    // derivados (c/dark/light/leg) que el resto de este switch ya calculaba.
    case "stairs": {
      out.push(...buildStairsArchetype(wS, dS, H, c, light));
      break;
    }
    case "railing": {
      out.push(...buildRailingArchetype(wS, dS, H, c, dark, light));
      break;
    }
    case "sofa": {
      out.push(...buildSofaArchetype(wS, dS, H, c, dark, leg));
      break;
    }
    case "bed": {
      out.push(...buildBedArchetype(wS, dS, H, c, dark, light));
      break;
    }
    case "toilet": {
      out.push(...buildToiletArchetype(wS, dS, H, c, light));
      break;
    }
    case "sink": {
      out.push(...buildSinkArchetype(wS, dS, H, c, dark, light, leg));
      break;
    }
    case "zone":
    case "path":
