...
/**
 * Vista general del dibujo (overview minimap) para el editor CAD.
 *
 * En un plano grande la cámara se aleja del trabajo y el dibujante pierde la
 * referencia de dónde está. Esta miniatura dibuja el contorno del plano, los
 * objetos colocados y el punto al que mira la cámara, y deja hacer clic en
 * cualquier parte para recentrar la vista sin perder el zoom. Es el equivalente
 * del panel de navegación de un CAD de escritorio.
 *
 * Como el `Minimap` 2D, lee las refs del editor dentro de un rAF acotado y
 * fotografía un modelo de vista mínimo en estado — el render nunca toca refs —,
 * así que sus refrescos frecuentes no re-renderizan el editor pesado.
 */
...
export default function CadOverviewMinimap({
  ctxRef,
  placementsRef,
  assetsRef,
  cameraRef,
    ...
    const tick = (t: number) => {
      if (t - lastT > 90) {
        ...
        if (ctx && ctx.W && ctx.H) {
          ...
          const tgt = ctrl
            ? targetToWorld(ctrl.target.x, ctrl.target.z, ctx.s, ctx.W, ctx.H)
            : { x: ctx.W / 2, y: ctx.H / 2 };
          const camW = cam
            ? targetToWorld(cam.position.x, cam.position.z, ctx.s, ctx.W, ctx.H)
            : tgt;
          const sig = `${scale.toFixed(5)}|${tgt.x.toFixed(0)},${tgt.y.toFixed(0)}|${camW.x.toFixed(0)},${camW.y.toFixed(0)}|${placements
            ...
            .join(
              ";",
            )}|${assets.map((a) => `${a.x},${a.y},${a.w},${a.h}`).join(";")}`;
          if (sig !== lastKey) {
            lastKey = sig;
            setSnap({
              scale,
  ...
  const navigate = (e: React.MouseEvent<SVGSVGElement>) => {
    ...
    const wpt = clampToFootprint(
      (e.clientX - rect.left) / snap.scale,
      (e.clientY - rect.top) / snap.scale,
      ctx.W,
      ctx.H,
    );
