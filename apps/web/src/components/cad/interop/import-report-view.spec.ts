...
// --- el orden es el mensaje --------------------------------------------------
{
  ...
  assert.equal(
    sections[0].count,
    2,
    "se declaran las dos entidades que no entraron",
  );
  ...
  assert.deepEqual(
    clean.map((section) => section.fidelity),
    ["kept"],
  );
}
...
// --- el HTML que de verdad se ve --------------------------------------------
{
  ...
  const html = renderToStaticMarkup(
    createElement(CadDxfImportReportPanel, {
      report,
      fileName: "estructura.dxf",
    }),
  );
  assert.ok(
    html.includes("estructura.dxf"),
    "el informe se ancla al archivo que llegó",
  );
  assert.ok(html.includes('data-tone="alert"'), "el tono viaja al marcado");
  assert.ok(
    html.includes("No entró en el dibujo"),
    "el encabezado de pérdidas está",
  );
  ...
  const lostTag =
    html.match(/<details[^>]*data-testid="cad-dxf-import-lost"[^>]*>/)?.[0] ??
    "";
  assert.ok(lostTag, "la sección de pérdidas se renderiza");
  assert.ok(/\bopen\b/.test(lostTag), `debía salir desplegada: ${lostTag}`);
  const keptTag =
    html.match(/<details[^>]*data-testid="cad-dxf-import-kept"[^>]*>/)?.[0] ??
    "";
  assert.ok(
    !/\bopen\b/.test(keptTag),
    `lo conservado puede ir plegado: ${keptTag}`,
  );
  ...
  for (const jargon of [
    ...
    assert.ok(
      !visible.includes(jargon),
      `jerga visible en el panel: ${jargon}`,
    );
