 ...
 */
export function dxfPointToFootprint(
  nx: number,
  ny: number,
  model: DxfModel,
  p: DxfPlacement,
...
export function dxfToWalls(
  ...
  const thickness =
    Number(options.thickness) > 0 ? Number(options.thickness) : 150;
  const minLen =
    Number.isFinite(options.minLen as number) && (options.minLen as number) >= 0
      ? (options.minLen as number)
      : 60;
  const maxWalls =
    Number(options.maxWalls) > 0 ? Math.floor(Number(options.maxWalls)) : 1500;
  const simplify = options.simplify !== false;
  const collinearDeg = Number.isFinite(options.collinearDeg as number)
    ? (options.collinearDeg as number)
    : 6;
