 ...
 *
 * - Aquí: la maquinaria —lanzar sondas, agregar medianas, escribir el fichero—.
 * - `curve-kernel-render-contract.mjs`: la REGLA por la que un artefacto se
 *   acepta o se rechaza. Vive aparte para poder leerse y probarse sin abrir el
 *   benchmark; este archivo la importa y la reexporta, no la copia.
 * - `curve-kernel-render-probe.mts`: una corrida, un proceso, JSON a stdout.
 * - `curve-kernel-render-bench.spec.mjs`: le mete artefactos rotos al contrato
 *   y exige que los rechace uno a uno, y después lee el publicado.
 *
 * Uso:
...
import { EVIDENCE_FILE, verificarArtefacto } from "./curve-kernel-render-contract.mjs";

// Se reexporta para que quien ya importa el generador siga teniendo a mano el
// contrato: la regla vive en un archivo y se lee desde el otro, no se copia.
export { EVIDENCE_FILE, verificarArtefacto };
