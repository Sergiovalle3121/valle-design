...
import {
  ...
  projectRecord,
  TOLERANCE,
} from "./corpus-entity-compare.mjs";
import {
  DwgCorpusGateError,
  fetchAdmittedCorpus,
  loadCorpusPin,
...
async function main() {
  ...
  for (const bundle of bundles) {
    ...
    for (const fixture of fixtures) {
      ...
      // LOS DOS ESPACIOS JUNTOS, A PROPÓSITO. Desde 2026-09-02 el lector
      // separa paper space del modelo, pero este oráculo NO lee el grupo 67
      // del DXF —el que marca la presentación—, así que su lista de entidades
      // de primer nivel mezcla las dos. Comparar sólo el modelo daría una
      // discrepancia falsa: en `23-layout-viewport` faltarían los dos
      // VIEWPORT que el DXF sí enumera. Que la separación es CORRECTA lo mide
      // `probe-paper-space.mjs` contra ese mismo grupo 67, que es su sitio.
      const readTop = flattenRecords([
        ...database.modelSpaceEntities,
        ...database.paperSpaceEntities,
