...
import { createDwgHandle, sameDwgHandle } from "../../src/model/handle.js";
...
test("neutral handles own immutable byte values", () => {
  ...
  bytes.fill(9);
  assert.deepEqual(created.bytes, [1, 2, 3]);
...
test("neutral byte factories ignore hostile public properties and iterators", () => {
  ...
  assert.deepEqual(createDwgHandle(bytes, 1).bytes, [7]);
  assert.deepEqual(
...
test("opaque objects retain bytes without interpreting active content", () => {
  ...
  payload.fill(0);
  assert.deepEqual(object.rawPayload, [104, 116, 116, 112, 58, 47, 47]);
