...
| better-sqlite3 13 | **Medido el 2026-09-23 sobre el PR #241** (corrida `35909579367`): los workers de Jest del API mueren con `SIGSEGV` en 22 suites que abren SQLite (`cad.controller.spec`, `identity.integration.spec`, `organizations.integration.spec`…). Es un módulo nativo: el binario de la 13 no convive con el runtime Node 20 del CI. Además `typeorm` declara `peerOptional better-sqlite3 ^12`. El #241 se cerró sin fusionar el 2026-09-24. | una versión de better-sqlite3 13 que no crashee en Node 20 (o subir el runtime) y un typeorm que acepte 13; medir con `npm test --workspace=valle-design-api` |
