...
Los nombres de plan de este documento son los del catálogo vendible
(`Prueba`, `Individual`, `Despacho` — `apps/api/src/modules/commercial/commercial-catalog.bootstrap.ts`),
no los de una propuesta comercial aparte: `Prueba` es el trial de 14 días sin
compromiso, `Individual` el plan de un asiento, `Despacho` el plan por
asientos (3 o más). `/sla` publica este mismo documento y los nombres los lee
del catálogo en vivo, así que no pueden desincronizarse en silencio.
...
| -------------------------------------------------- | ---------------- | ------------------ | ---------------------- |
| Objetivo de disponibilidad mensual                 | _sin compromiso_ | 99,5 %             | 99,9 %                 |
...
**Prueba no lleva compromiso de disponibilidad, y es una decisión honesta.**
...
| Severidad | Prueba              | Individual     | Despacho      |
| --------- | ------------------- | -------------- | ------------- |
...
- **No hay compromiso de disponibilidad para el plan Prueba** (§2).
