> **EN PAUSA (17-sep 11:00):** la auditoria de tu noche encontro 57 comandos de relleno y 4 trampas.
> Hasta cerrar **T18** de la cola no se anade ningun comando nuevo. Lee T18 antes que esto.
>
> **COMO SE LEVANTA ESTA PAUSA — 17-sep 17:55. No esperes a que nadie te lo diga.**
> Sergio esta fuera dos dias y el supervisor puede no estar. La pausa se levanta SOLA en cuanto
> tus commits esten en `main`. Compruebalo tu mismo, al principio de la vuelta:
>
>     gh pr view 209 --json state,mergeStateStatus --jq '.state + " " + .mergeStateStatus'
>
> - Dice `MERGED ...` -> **la pausa esta levantada.** Sigue con esta mision desde la Fase 0 de
>   abajo, sin pedir permiso. T18 queda cerrado por el hecho del merge.
> - Dice `OPEN BLOCKED` o `OPEN ...` -> el CI sigue rojo o la corrida no ha terminado: tu unica
>   tarea es ponerlo verde. Lee `.mimocode/bloqueos-actuales.md` y `.mimocode/ci-fallo.md`. Cero
>   comandos nuevos mientras tanto.
> - El comando falla o no hay red -> mira si `git log --oneline origin/main -1` contiene ya tu
>   trabajo; si tu rama ya esta dentro de main, la pausa esta levantada igual.
>
> Y si la #209 aparece `MERGED` pero la rama tiene commits nuevos encima, el lanzador abrira otra
> PR sola: la regla es la misma, verde primero, comandos despues.
