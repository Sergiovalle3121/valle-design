...
| `SENTRY_DSN`      | No        | Activa el adaptador HTTP compatible con Sentry (sin dependencia nueva; envío por `fetch`). **Sin él el reporte de errores es INERTE**: cero red. Un DSN explícito pero inválido impide arrancar en producción sin imprimir su valor; en desarrollo registra el motivo y usa el adaptador nulo. La configuración válida aún requiere comprobar una entrega real. |
...
| `NEXT_PUBLIC_SALES_URL`, `NEXT_PUBLIC_DOCUMENTATION_URL`, `NEXT_PUBLIC_PRICING_URL`, `NEXT_PUBLIC_SUPPORT_URL`, `NEXT_PUBLIC_STATUS_URL`, `NEXT_PUBLIC_CONTACT_URL`, `NEXT_PUBLIC_PRIVACY_URL`, `NEXT_PUBLIC_TERMS_URL`, `NEXT_PUBLIC_LICENSES_URL` | Enlaces comerciales opcionales. Sin URL se usan páginas internas existentes; no apuntes a destinos ficticios. |
| `NEXT_PUBLIC_SITE_URL`                                                                                                                                                                                                   | Origen público del propio despliegue (sitemap, `robots.txt`, canonical). Sin configurar cae a `NEXT_PUBLIC_BRAND_WEBSITE_URL`.  |
| `NEXT_PUBLIC_BRAND_*`                                                                                                                                                                                                    | Marca y textos. La mayoría es opcional y no cambia capacidades ni entidad legal real, pero `..._WEBSITE_URL`, `..._SUPPORT_EMAIL`, `..._SALES_EMAIL` y `..._PRIVACY_EMAIL` son OBLIGATORIAS en un build de producción real: `apps/web/scripts/check-production-config.mts` (enganchado en `apps/web/Dockerfile`) revienta el build si quedan en su default de desarrollo (`*.invalid`). |
