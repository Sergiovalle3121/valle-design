...
| El punto del RATÓN bajo un SCU inclinado sale del plano de trabajo (`modeling3d.z-pointer`) | 0 | ninguna verde: `refutacion-scu-raton.spec.ts` sigue rojo en `e2e/auditoria/` (punto 1 aplanado a cota 0 con `LINE` desde la paleta); dibujar en la fachada con el ratón y `LINE` tecleada SÍ funciona (golden 101, graduado hoy) | `modeling3d.z-roundtrip` cobraba 2 pt con specs unitarios que ese spec de navegador contradecía; se partió en la frontera del DXF (5, con oráculo ajeno) y el puntero (0). Ficha T-52, racimo A; y **272 comandos de 294 no declaran nada** bajo un SCU inclinado. **Todavía no.** |
