...
| `FLATSHOT`/`SECTION`/`SLICE` sobre la entidad `wall` que emite `WALL` | 0 | ninguna: `flatshot-solids.ts` descarta todo lo que no sea `solid3d`/`box`/`station` antes de `volumeFor`, y `selectedSolids` filtra `entity.type === "solid3d"` | Es la ficha T-33 de la campaña «El lunes de un arquitecto»: el cuerpo B-rep del muro existe (`wall-solid-three.ts`) y sólo lo consumen dos módulos. Golden nuevo: tres muros con `WALL`, `FLATSHOT`, y el alzado tiene aristas del muro. **Todavía no.** |
