...
| Las capas de la xref sobreviven (`xrefs.layers`) | 0 | ninguna: `xref/xref-projection.ts` sobrescribe la capa de cada entidad con `xref:<id>:layer` de color y grosor fijos | La fila `xrefs.resolution` cobraba 2 pt por «capas de xref» con una sola spec de bind; se partió en bind (5) y capas (0). Ficha T-41: apagar una capa del estructurista y que las otras sigan. **Todavía no.** |
