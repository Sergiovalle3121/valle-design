...
| MEP (mitad 2D) | 5 | 3 | PIPE, DUCT, CABLETRAY y MEPSYMBOL sobre el mismo motor (polilíneas, capas de servicio con tipo de línea con texto, bloques MEP-…), y el cuadro de instalaciones por DATAEXTRACTION Instalaciones en la lámina (golden 81; `mep-tracing.spec.ts`). Con cota desde la ventana 2 de la campaña (2026-09-04): `Elevación` y montante en las tres órdenes, longitudes en TRES dimensiones —un montante de 2 m sumaba cero metros— y choques contra muros, huecos y sólidos del propio dibujo por distancia exacta (`mep-tracing.spec.ts`, 127; `plant/clash.spec.ts`, 56). Queda fuera el diámetro por especificación y el canto del ducto, que el formato no guarda: la holgura vertical de un ducto es la de su ancho, y se dice. |
...
| Cuadro de superficies con el NOMBRE del local (el rótulo TEXT/MTEXT dentro del anillo), su uso y las TRES áreas: a ejes, útil y construida | 5 | `bim-schedule.spec.ts` (66), `bim-areas.spec.ts` (41), `data-extraction.spec.ts` (25: 12,00 / 10,64 / 13,44 m² en un cuarto de 4 × 3 con muros de 200), `data-extraction-commands.spec.ts` (27); golden 77: RECÁMARA 16,00 m², BAÑO 8,00 m² | Un local sin rótulo sigue llamándose por su `L-nn`. El uso sale del clasificador de `architecture.ts`; un nombre que no reconoce da «—». La construida sale a cara exterior del muro perimetral y al EJE del medianero —para que los locales SUMEN la huella de la planta— pero no descuenta patios ni el hueco de escalera (nadie declara qué cara tiene techo) ni se suma por NIVEL: hay una planta, no un edificio. |
...
| El diálogo «Exportar a DXF» escribe las capas con su tipo de línea y grosor, la tabla LTYPE y $LTSCALE | 5 | golden 80; `layout-export-adapter.spec.ts` | Medido antes: GAS = GAS_LINE salía como `6 CONTINUOUS` sin aviso. Desde el 2026-09-05 el color de la capa sale del DOCUMENTO (`hexToAci`), no de una tabla fija: hasta ese día el fichero devuelto llevaba `62 7` en todas y el plano del remitente volvía monocromo. Siete de los 255 índices ACI vuelven con otro número —los duplicados de la propia paleta, 1/10, 2/50 … 7/255, el mismo RGB—, así que cambia la etiqueta y no el color; medido sobre los 255. |
| PIPE por servicio (agua Fría, agua Caliente, Sanitario, Pluvial, Gas, contra Incendio) con Diámetro | 5 | golden 81; `mep-tracing.spec.ts` (127) | Sin accesorios automáticos (codos, tes, reducciones), sin pendientes ni tramos curvos, con ruteo 3D por cota y montante, sin diámetros por especificación (no se transcribe ningún catálogo y el del proyecto sigue en la cola, sin entregar). |
...
| --- | --- | --- | --- |
| `FLATSHOT`/`SOLPROF` sobre el muro HEREDADO (`{type: "box", kind: "wall"}`), columnas y mobiliario del catálogo de arquetipos | 5 | golden 92 sobre el documento que recibe el servidor: `UCS X 90` + `FLATSHOT` sobre dos muros en L deja el alzado con la altura del muro y cuenta lo excluido con su motivo; `flatshot-solids.spec.ts` (30) | Nada de peldaño para lo que hace. La altura sale del catálogo de arquetipos del visor: un `kind` que no está en él se queda fuera Y se cuenta, en vez de salir con la altura de otra cosa. **Corrección del 2026-09-06:** esta fila decía «sobre el modelo del ARQUITECTO (muros…)» y el golden 92 monta el muro heredado, no la entidad que `WALL` emite. Se separa en la fila siguiente. |
| `FLATSHOT`/`SECTION` sobre la entidad `wall` que emite `WALL` | 5 | golden 140 (`e2e/golden/140-cad-alzado-muro-nativo.spec.ts`) sobre el documento que recibe el servidor: `UCS X 90` + `FLATSHOT` sobre TRES muros nativos en L con una puerta alojada deja el alzado con la altura DEL MURO (la de la entidad, no la de un catálogo de arquetipos) y el dintel de la puerta a su cota; `flatshot-solids.spec.ts` (secciones 9-12). El golden 92 (muro heredado) sigue siendo evidencia de que el `box` de compatibilidad TAMBIÉN funciona, pero no es la de este peldaño | `SECTION` ya lee el mismo cuerpo del muro nativo (compartido con FLATSHOT vía `cadWallWorldBody`, `flatshot-solids.ts`); **`SLICE` sigue sin muros a propósito**: convertiría la entidad en `solid3d` horneado y rompería «wall and opening stay parametric» de `AGENTS.md`. Corregido el 2026-09-06 por la petición F5-P-02. |
...
| Símbolos de esquema de control (IEC 60617: bobina, contacto NA/NC, contactor, guardamotor, seccionador) | 0 | ninguna: `grep -rniE 'bobina|contactor|relevador|guardamotor|seccionador|60617' apps packages` no devuelve un solo símbolo de esquema; los ocho de `mep-symbols.ts` son de INSTALACIÓN en planta | El criterio `toolset-electrical.esquemas` de la rúbrica sólo verifica numeración y etiquetado (su texto lo dice desde el 2026-09-06, petición F5-P-01, opción A). Construirlos como bloques con `attributes` y su golden es una ficha futura. **Todavía no.** |
...
| Ruteo de tubería 3D con cota y especificación | 5 | `pipe-route.spec.ts` (26) y `plant-route.spec.ts` (26): la longitud es la de TRES dimensiones —un montante de 3 m no es cero metros de tubo—, cambiar de elevación mete el vértice del montante, y la ruta llega al documento con z en cada vértice; golden 95 lo afirma sobre lo persistido | El comando se declara `spatial: true` porque conserva la cota de punta a punta. Desde 2026-09-04 la palabra clave `Sólido` emite el cuerpo del tubo en el mismo lote de deshacer y se ve en el visor 3D y en FLATSHOT (`pipe-solid.spec.ts`, 77): es un prisma FACETADO de 16 lados de área equivalente al diámetro NOMINAL, no un cilindro y no el exterior. Lo que falta es el diámetro EXTERIOR —una `6"` mide más de 152,4 mm por fuera— y cuánto más lo dice un catálogo con dueño. **Todavía no.** |
...
| Isométrico con longitudes VERDADERAS, norte y hoja | 5 | `isometric.spec.ts` (26): ejes a 30°/150°/vertical, el codo recto dibujado de 120°, y la diagonal que se dibuja de 1.000 midiendo 1.414 —el caso donde una cota mentiría—; `plant-iso.spec.ts` (22): dibujo, cuadro y HOJA en un paso, con la ventana encuadrando los dos; golden 95 | Por eso la longitud va como TEXTO y el título declara `SIN ESCALA`. Falta la salida en el formato de ISOGEN, y el motivo se dice (2026-09-04): es un formato PROPIETARIO, sin especificación pública ni oráculo con el que comprobar una salida. El isométrico propio sí existe y es el que se dibuja; la hoja es del documento. **Todavía no.** |
| Isométricos | 0 | ninguna | Sin ruteo 3D no hay de dónde sacarlos. **Todavía no.** |
| Catálogo de fabricante con claves y precios | 0 | ninguna | El metrado sale; la requisición valorada no. Lo que falta, con nombre y apellidos (2026-09-04): espesor de pared, diámetro exterior, peso, clave de compra y precio. Está en normas y catálogos con dueño y **no se transcribe ninguno**. El catálogo del PROYECTO —el que la organización escribe y amplía— es el camino elegido y **tampoco existe todavía**: sigue en la cola del frente, sin entregar. Mientras no exista, la holgura de choque sale OPTIMISTA por el grosor de pared y el aislamiento, y el tubo se modela macizo. **Todavía no.** |
...
| Volumen de tubería DERIVADO en el visor, como el del muro | 0 | ninguna para la derivación; sí para el aviso: `pipe-solid.spec.ts` (77) y `plant-route.spec.ts` (49) mueven un vértice y PIDMTO declara el sólido viejo, o huérfano si se borró la ruta | El cuerpo se emite como `solid3d` con su receta de barrido y se PERSISTE; no se deriva de la polilínea en cada dibujado (2026-09-04). Mover la ruta no mueve el sólido: se AVISA, no se regenera, y regenerar pediría una orden nueva. **Todavía no.** |
...
## La campaña «El lunes de un arquitecto» — Ola 0 (2026-09-06)

La auditoría de veinte dimensiones (`docs/execution/auditoria-fable/`) puso
**5,15/10** al mismo producto que la rúbrica mide en torno al 90 %: las dos
cifras son correctas y miden cosas distintas —si la capacidad existe con
evidencia, y si un profesional puede usarla el lunes—. Cuatro dimensiones
enteras no tenían dónde puntuar. La Ola 0 abre las filas **antes** de
construir, para que el trabajo se pueda cobrar y el `gap` diga la verdad
mientras tanto; el porcentaje **baja** porque el denominador crece con filas
honestas, y eso es un éxito, no una regresión. La cifra la computa
`node scripts/cad/rubric.mjs`; aquí no se copia.

| Capacidad | Peldaño hoy | Evidencia | Qué falta para subir |
| --- | --- | --- | --- |
| Comprar, crecer, entrar y salir (grupo `comercial`: compra y asientos, facturación declarada, expediente del comprador, migración) | 3 en lo que existe (checkout con tarjeta/OXXO/SPEI, asientos en el servidor, sólo lectura al vencer, reembolsos y disputas por webhook) y, desde F8 (2026-09-06), en comprar el asiento que falta (T-61), «Factura CFDI» derivada del modo real del proveedor (T-18a), los ocho correos del árbol con renderizador y gate de cobertura (T-17, `identity.new-sign-in` primero) y arrastrar un DXF al estado vacío, al tablero (T-63f) y al lienzo del estudio (petición F8-2, por la puerta compartida de T-16); y desde la segunda cabeza de F8 (T-62), en el expediente del comprador: la bitácora de auditoría se LEE (`GET /v1/organizations/{id}/audit-log`, pantalla en `/equipo`, aislamiento por inquilino probado contra PostgreSQL, retención con purga), `/sla` publica los nombres de plan del catálogo y los datos personales se exportan (`GET /v1/auth/export`); 0 en la salida | `commercial-seat-growth.pg.spec.ts`, `commercial-surface.spec.ts` + `public-catalog.controller.ts`, `email-template-coverage.spec.ts` + `email-templates.ts`, goldens 150 y 196; los specs de `apps/api/src/modules/commercial/` y `entitlement-read-only.pg.spec.ts`; `audit-log-tenant-isolation.pg.spec.ts`, `sla-surface.spec.ts`, `identity-export.pg.spec.ts` | Borrar la cuenta (la otra mitad de ARCO; exportar ya existe) y el precio comparado con fecha y fuente (T-62 c/d, peticiones F8 §4-5); el proyecto implícito del primer minuto (T-63); que el servidor exija y registre la aceptación de términos al registrarse (T-63d parte 2, F8: la casilla del alta ya nombra la versión vigente y bloquea «Crear cuenta» hasta marcarla, golden 197); la salida completa (T-11 a, en F4). **Todavía no** en esas filas. |
| Lo que sólo un CAD en el navegador puede dar (grupo `navegador`: el plano entregado tiene dirección, el invitado existe entre máquinas, un punto del dibujo tiene URL, se instala y avisa) | 1 en las piezas (enlace server-owned, QR propio, presencia, llamada); 0 como capacidad | `review-link.service.ts`, `qr-roundtrip.spec.ts`, `presence.spec.ts`, `comment-anchor.spec.ts` | Coser las piezas: los bytes de la lámina al lado del recibo, el enlace sin sesión que enseña la LÁMINA con versión, `sha256` y pérdidas, revocar → 403, el invitado visible y en la llamada, el punto con URL, push (T-43, T-42). **Todavía no.** |
| Degradación honesta (`degradation`, en integridad): el 400 permanente no se vende como transitorio; la profundidad de deshacer prometida es la entregada; ninguna operación deja el documento fuera del límite del servidor; el almacenamiento local avisa cuando deja de proteger | 3 en la maquinaria de abajo (CAS atómico, diario de recuperación, `gunzipBounded`, ventana de pérdida medida); 0 en las cuatro promesas | `save-failure.spec.ts` (cinco estados), `cad-recovery.spec.ts`, `document-limits.json` | El caso 400 (T-24·3); `undoDepthByTier` medido y en la barra de estado (T-24·2); confirmación de `ARRAY`/`COPY`/`DIVIDE` sobre `maxEntities` (T-24·1); `openDatabase` bloqueado con aviso (T-75 c). **Todavía no.** |
| La cinta y el gesto (`ribbon-ux`): pestaña contextual, KeyTips, sugerencias en la línea de comandos, menú contextual por designación, pestañas de láminas completas con Ctrl+RePág, sólo lectura que no apaga lo que no muta | 0 | ninguna | Ficha T-74. `check-ribbon-coverage.mjs` y `ui-command-reach.mjs` se amplían a pestaña contextual y KeyTips. **Todavía no.** |
| Sin ratón, sin vista y en dos idiomas (`accessibility`): axe con `moderate` como rojo, foco visible a cero, roles de la cinta, el prompt vivo releíble (F2), `forced-colors`, vocabulario de comandos en español | 0 | ninguna (`prefers-reduced-motion` es el modelo a copiar) | Ficha T-73. **Todavía no.** |
| Las capas de la xref sobreviven (`xrefs.layers`, T-41) | **arreglada**: `xref-projection.ts` sobrescribía la capa de cada entidad con una sola `xref:<id>:layer` gris de grosor fijo. Ahora `projectCadXrefLayers` proyecta cada capa del plano ajeno como `XREF|<xref>|<capa>` (la convención de AutoCAD) con su color, tipo de línea y grosor, y la entidad conserva su capa; adjuntar las da de alta en el anfitrión y desligar las retira reasignando a una capa que sobrevive. Son capas del anfitrión: se apagan, se congelan y se bloquean desde el gestor, y el estado viaja con el documento (VISRETAIN). El gestor rechazaba apagar una capa cuyo nombre lleva `|`: el filtro de caracteres DXF corre sólo al renombrar | golden 210 (XATTACH tecleado de un plano de tres capas: el gestor lista seis, apagar EJES deja MUROS y TEXTOS encendidas en pantalla y en el documento que recibe el servidor, con el color del remitente y tres capas dentro del bloque); `xref-workflow.spec.ts` (caso 6: adjuntar da de alta las capas, desligar las retira) y `cad-layer-manager.spec.ts` (apagar y bloquear `XREF|x|y` sin renombrar; renombrar con `|` sigue prohibido) | Atenuar la xref (`XDWGFADECTL`) sigue sin existir: `fade` es sólo de ráster. Las capas proyectadas se listan entre las del anfitrión, sin filtro ni árbol por xref, y el cuadro XREF sigue sin opción de capas. **Todavía no** en eso. |
| El punto del RATÓN bajo un SCU inclinado sale del plano de trabajo (`modeling3d.z-pointer`, T-52 racimo A) | **arreglada**: el rayo ya cortaba el plano de trabajo, pero el imán perdía la cota: el enganche 3D proyectaba la SOMBRA del punto en el suelo y enganchaba la arista de abajo, y los candidatos 2D devolvían un punto sin z. Ahora `snapAtDrawingPoint` proyecta el punto real con su cota, bajo un plano inclinado sólo engancha lo que está EN el plano (`cadDistanceToUcsPlane`, dentro de la apertura), y las sombras 2D y el rastreo se saltan | golden 198 (LINE desde la paleta, dos clics sobre la fachada: y exacta y cota positiva en los dos puntos) y golden 101 (LINE tecleada); `solid-shade-host.spec.ts` (con cota engancha la arista de arriba; sin cota, la sombra enganchaba la de abajo) y `pointer-work-plane.spec.ts` (distancia al plano) | La rejilla sigue redondeando x e y en planta: bajo un plano que no sea vertical ni horizontal, con la rejilla de captura encendida el punto puede salirse del plano — declarado. El índice 3D no sabe qué tapa el sólido (por eso el filtro al plano); y **272 comandos de 294 no declaran nada** bajo un SCU inclinado. **Todavía no** en eso. |
| Designar en 3D va por el RAYO de cámara, no por la sombra en el suelo (T-52 racimo B) | **arreglada**: el puente del motor definía `hitEntity` sobre el punto del suelo bajo el cursor y consultaba el índice 2D, así que con un comando pidiendo objeto el clic sobre un sólido elevado designaba lo que hubiera bajo su SOMBRA. `hitEntityAt` lanza el rayo de cámara contra la escena (el mismo raycast de la selección libre) y va antes que el pickbox del suelo, que sigue mandando en 2D y cuando el rayo no toca nada | golden 199 (ERASE + clic en el centro del lienzo en isométrica: se borra la caja elevada que se ve, no la línea que pasa bajo la sombra del cursor); `pointer-router.spec.ts` (9b: el rayo va antes que el pickbox y cae a él con `null`) | Los sólidos siguen sin designación de subobjeto (arista, cara fuera de un comando) y el rayo no distingue capas bloqueadas: lo que la selección libre ya hacía. Y **272 comandos de 294 no declaran nada** bajo un SCU inclinado. **Todavía no** en eso. |
| El imán devuelve el punto con su nombre (`draw-2d.osnap`) | **defecto declarado**: con CEN encendido el cursor se pega al punto MEDIO de toda línea y de todo eje de muro y el HUD anuncia «centro»; con NOD, al medio de cada tramo de polilínea y anuncia «nodo» | `entity-runtime.ts` (`CadSnapKind` tiene cinco valores) y el reparto «todo lo demás → endpoints» de `snap-scene.ts`; `professional-snapping.spec.ts:22-42` es verde sobre una escena escrita a mano que ningún adaptador alimenta | Es un fallo de **integridad**, no de comodidad, y hasta hoy no estaba cobijado por ningún «todavía no». Ficha T-14 (frente F3): `midpoint`, `node`, `insertion` y `geometric-center` como kinds reales servidos por los adaptadores del documento, y el spec alimentado por ellos. |
| Importar un DXF de más de 12 MB | **límite declarado**: `MAX_DXF_IMPORT_BYTES = 12_000_000` (`document-import-validation.ts:27`) rechaza de plano; con la medida del propio proyecto (`floorplan.dxf`, 1,1 MB para 961 entidades) son unas 10 000 entidades, y un plano ejecutivo real pasa de 50 000 | el propio tope y su prueba en `document-import-validation` | AutoCAD no tiene ningún tope de este tipo, y el flujo que el producto recomienda al rechazar un DWG («pídele el DXF») falla por tamaño. O se sube con lectura en flujo y se mide, o se queda declarado aquí (ficha T-16): queda declarado. Desde T-16 la puerta del estudio (`onDxfFile`) mide ESTE tope, en bytes y antes de leer, por la misma función que el tablero; el tope a mano en unidades UTF-16 desapareció. Subirlo o medir en flujo sigue siendo **todavía no.** |
| El invitado del enlace oye la verdad sobre quién más mira (`navegador`, presencia) | **arreglada**: `connected` sólo se afirma con un transporte que alcance otras máquinas (`!guest && serverChannelAvailable`, `presence-affirmation.ts`); al invitado —al que el servidor no se le abre— el panel ya no le dice «Nadie más en este documento ahora mismo» sino «No se puede saber quién más está mirando desde aquí» | `presence-affirmation.spec.ts` (12 comprobaciones: regla y cableado del hook y del panel); golden 109 sin cambio para la arquitecta first-party | Que el invitado exista de verdad entre máquinas (SSE con token de revisión) es la ficha T-43 completa; esto sólo quita la mentira (D1). **Todavía no** como capacidad. |
| Volver a una versión anterior del servidor desde el estudio («Versiones», T-12·1) | **arreglada**: el cuadro lista el historial CAS real (`GET /v1/cad/documents/:id/versions`, una versión por guardado, de más nueva a más vieja, con quién y cuándo) y «Restaurar» guarda esa versión como versión NUEVA con el CAS de la cabeza que el editor conoce (409 explicado si el servidor avanzó; se niega con cambios sin mandar o con un puntero a blob). Antes pedía `layout/snapshots…`, ruta que el adaptador declaraba «404 limpio»: lista siempre vacía y tres botones que sólo sacaban error. Ya no promete «guardar versión» ni borrar: el historial es inmutable y las versiones con nombre viven en la paleta de colaboración | golden 191 (`e2e/golden/191-auditoria-versiones-servidor.spec.ts`: dibujar → versión 2, restaurar la 1 → versión 3 con el plano de la 1, la 2 sigue en la lista); `versions-host.spec.ts` (28 comprobaciones: orden, identidad, CAS de cabeza, sucio, puntero, 409, sólo lectura, cableado) | Restaurar SIN pasar por el guardado (previsualizar una versión antes de decidir) y el diff visual entre dos versiones del servidor: la paleta de colaboración compara versiones con nombre, no el historial CAS. **Todavía no.** |
| Evidencia independiente en seis filas que la retenían (frente F10: `geo`, `toolset-map3d`, `api-sdk`, `events`, `wasm`, `json-import`) | **verificada por testigo ajeno**: pyproj/PROJ (malla de control de México y un shapefile público de Natural Earth), openapi-spec-validator (el YAML contra el esquema oficial de OpenAPI 3.1), la biblioteca estándar de Python (HMAC-SHA256 recalculado desde cero), mpmath (arcos y elipses a 50 dígitos como referencia absoluta del kernel) y atheris (fuzzer de cobertura ajeno decidiendo los documentos hostiles); cada uno con censo congelado en `docs/cad/corpus/oraculos/` y licencia hasheada | `crs-pyproj.spec.ts`, `shapefile-pyproj.spec.ts`, `api-sdk-openapi.spec.ts`, `events-hmac.spec.ts`, `curve-kernel-mpmath.spec.ts`, `json-import-atheris.spec.ts` (marcados `independent: true`; el censo `independencia-por-fila.json` los admite y retira sus seis dictámenes); la cifra la imprime `node scripts/cad/rubric.mjs` | Las filas que aún retienen 1 pt las lista el censo; `object-storage.s3` espera la decisión del titular sobre MinIO (AGPL sólo por red, D-10); el oráculo de PDF (pypdf/pdfminer) para la fila de trazado está encargado a F10. |
| «Exportar imagen (PNG)» entrega lo que hay en pantalla (T-12·2) | **arreglada**: el PNG se pinta con la cámara ACTIVA del controlador de vista (ortográfica en planta, perspectiva en volumen), la misma del bucle de render; antes forzaba la `PerspectiveCamera` cruda y en planta el PNG que se mandaba al cliente llevaba la deformación que `perspective-distortion.spec.ts` declara inaceptable | `export-scene-actions.spec.ts` (8: la regla `pickCadExportCamera` y el cableado: `exportPng` pide la cámara al controlador, el monolito la pasa, el bucle de render usa la misma) | Un golden que compare el PNG descargado con una captura del lienzo píxel a píxel (hoy se afirma la cámara, no los píxeles). **Todavía no** como golden. |
| «Previo» en «Designe objetos» recuerda el último conjunto (T-21, F3-P-02) | **arreglada**: el anfitrión del motor anota `session.lastSelectionIds` cuando un conjunto entra a un comando (`select`) o cuando QSELECT/FILTER lo designan; antes el campo existía y nadie lo escribía, así que «Previo» resolvía siempre a vacío | `command-engine-host-previo.spec.ts` (3: sin previo no designa; tras COPY sobre dos líneas, MOVE + «Previo» + Intro pasa al punto base) y `selection-keywords.spec.ts` (13) | Golden de navegador de F3 (rango 120-129) que lo haga con el ratón: ERASE de dos objetos y MOVE con «Previo». |
| El indicador U/R dice cuándo manda el presupuesto de memoria (T-24·2, F3-P-04) | **arreglada**: `cad-history-depth` lleva `data-history-floor` y un `title` que explica que en un plano denso (desde el escalón de `undoDepthByTier`) el historial sólo conserva uno o dos pasos, en vez de quedarse mudo en `U1/R0` | `history-depth-hint.spec.ts` (9) sobre `docs/cad/evidence/document-limits.json` (`undoDepthByTier`, `scripts/cad/undo-depth-benchmark.mts`) | Subir `maxRetainedBytes` o pasar a checkpoints diferenciales es una decisión de producto (memoria por pestaña) que queda para el titular; golden con el corpus `plano-real` pendiente. |
| Las dos puertas de importación contestan lo mismo al mismo archivo (T-16) | **arreglada**: el estudio decide con `admitStudioBackdropFile` → `validateImportFile`, por nombre y bytes, antes de leer nada: la misma función, las mismas betas y el mismo tope que «Importar como documento». Un `.dwg` con las puertas cerradas recibe en las dos pantallas la razón DWG del contrato (`dwg-unavailable-reason.ts`), palabra por palabra; antes el tablero decía «formato no soportado» y el estudio otra cosa, y con la beta encendida el estudio negaba lo que el tablero admitía | golden 192 (el mismo `.dwg` por las dos puertas con la beta apagada: la misma frase, y nada viaja al servidor); `document-import-door.spec.ts` (23: la tabla de nombres y tamaños por las dos puertas, con la beta apagada y encendida) | Con la beta encendida el fondo del estudio sigue sin pintar un DWG: lo dice y señala la puerta del tablero (D-12). Convertirlo a fondo (DWG → documento → DXF) es una ola aparte. |
| La inserción de un ATTDEF se imanta como inserción (F3-P-01) | **arreglada**: `attdefAdapter.snaps` emitía `kind: "endpoint"` para la inserción —el mismo error que el MTEXT tenía antes de T-14—, así que INS no la encontraba y END sí; ahora emite `insertion` | `professional-snapping.spec.ts` (22): un ATTDEF real en la escena que construye `cadSnapSceneAddEntities` resuelve `insertion` en su punto | Nada de peldaño. |
| El clic con el que se designa LLEGA al comando aunque caiga sobre un pinzamiento (T-20) | **arreglada**: el controlador de pinzamientos tenía derecho de tanteo sobre el clic sin preguntar si un comando esperaba algo; `CadNativeGripDeps.commandActive` pregunta lo que `CadCommandEngineHost.accepts` ya contestaba (0 en reposo) y `start()` cede: en reposo el pinzamiento gana, con un comando abierto el clic es del comando. OFFSET sobre el punto medio del eje designado y TRIM sobre el extremo del muro designado hacen lo que dicen | goldens 193, 194 y 195 (once casos, graduados de `e2e/auditoria/` con el techo 9 → 6): OFFSET lejos y sobre el pinzamiento, con y sin designar; TRIM en seis combinaciones de designación y punto, y un arrastre DURANTE el comando que no mueve nada; `native-grip-controller.spec.ts` (bloque 6) | Nada de peldaño. Un pinzamiento no se arrastra mientras un comando pide punto (como en AutoCAD); el recorrido de OFFSET se adaptó a T-23 (el lado se pincha) — D-13. |

| Las «frases» en español de Ctrl+K («coloca una puerta en 3000,2000», «medir de A a B»…) | **OCULTA** (fix-or-hide, 2026-09-06) | `command-palette.spec.ts` afirma que la paleta no ofrece ninguna entrada de frase; golden 119 lo mira en el navegador | Las cuarenta entradas prometían «Preview listo en el Copiloto CAD», un panel retirado con la IA (`no-ai-boundary.spec.ts` impide que vuelva) y cuyo «Aplicar» estaba muerto. El parser determinista (`commands/registry.ts`) sigue en el árbol para la línea de comandos. Volver a ofrecerlas exige cerrar el circuito bajo un nombre honesto: vista previa en `CadCommandLine` y ejecución sin panel, con golden. No es IA y no se finge que lo sea (D-04). |
| La palabra «imprimir» en Ctrl+K lleva a trazar | 5 | golden 190: buscar «imprimir» ofrece PLOT (resumen «Imprime la lámina…») y pulsarlo arranca PLOT con su opción «Trazar» | Antes la única entrada con la palabra era una frase que no sacaba nada. No hay alias `IMPRIMIR` en la línea de comandos: es el vocabulario en español de la ficha T-73 (c), «todavía no». |
