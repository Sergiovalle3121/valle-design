...
| Capacidad | Peldaño hoy | Evidencia | Qué falta para subir |
| --- | --- | --- | --- |
| Comprar, crecer, entrar y salir (grupo `comercial`: compra y asientos, facturación declarada, expediente del comprador, migración) | 3 en lo que existe (checkout con tarjeta/OXXO/SPEI, asientos en el servidor, sólo lectura al vencer, reembolsos y disputas por webhook) y, desde F8 (2026-09-06), en comprar el asiento que falta (T-61), «Factura CFDI» derivada del modo real del proveedor (T-18a), los ocho correos del árbol con renderizador y gate de cobertura (T-17, `identity.new-sign-in` primero) y arrastrar un DXF al estado vacío y al tablero (T-63f); 0 en el expediente del comprador y en la salida | `commercial-seat-growth.pg.spec.ts`, `commercial-surface.spec.ts` + `public-catalog.controller.ts`, `email-template-coverage.spec.ts` + `email-templates.ts`, golden 150; los specs de `apps/api/src/modules/commercial/` y `entitlement-read-only.pg.spec.ts` | Bitácora de auditoría legible, `/sla` atado al catálogo, derechos ARCO (T-62); el proyecto implícito del primer minuto y soltar el DXF sobre el lienzo del estudio (T-63, petición F8-2 al monolito); aceptar términos al crear la cuenta (T-63d, petición F8-1); la salida completa (T-11 a, en F4). **Todavía no** en esas filas. |
