...
| Importar un DXF de más de 12 MB | **límite declarado**: `MAX_DXF_IMPORT_BYTES = 12_000_000` (`document-import-validation.ts:27`) rechaza de plano; con la medida del propio proyecto (`floorplan.dxf`, 1,1 MB para 961 entidades) son unas 10 000 entidades, y un plano ejecutivo real pasa de 50 000 | el propio tope y su prueba en `document-import-validation` | AutoCAD no tiene ningún tope de este tipo, y el flujo que el producto recomienda al rechazar un DWG («pídele el DXF») falla por tamaño. O se sube con lectura en flujo y se mide, o se queda declarado aquí (ficha T-16). Mientras tanto la puerta del estudio (`onDxfFile`) tiene además su propio tope escrito a mano en unidades UTF-16 y no llama a `validateImportFile`: dos puertas, dos respuestas al mismo fichero. **Todavía no.** |
