...
| Capacidad | Peldaño hoy | Evidencia | Qué falta para subir |
| --- | --- | --- | --- |
| Comprar, crecer, entrar y salir (grupo `comercial`: compra y asientos, facturación declarada, expediente del comprador, migración) | 3 en lo que existe (checkout con tarjeta/OXXO/SPEI, asientos en el servidor, sólo lectura al vencer, reembolsos y disputas por webhook); 0 en el resto | los specs de `apps/api/src/modules/commercial/` y `entitlement-read-only.pg.spec.ts` | Comprar el cuarto asiento desde el producto (T-61); «Factura CFDI» derivada del descriptor del proveedor (T-18); gate de cobertura de plantillas de correo (T-17); bitácora de auditoría legible, `/sla` atado al catálogo, derechos ARCO (T-62); el primer minuto con el dibujo dentro y arrastrar-y-soltar (T-63); la salida completa (T-11 a). **Todavía no.** |
