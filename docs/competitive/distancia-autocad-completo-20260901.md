...
2. **Tabla de patrones de sombreado.** Sustituir `angles = [entity.angle]` por `familias = tablaDePatrones(nombre)` y llamar al generador una vez por familia, en dos sitios: `hatch-entity-adapter.ts:113` y `hatch-publish-strokes.ts:86`. Veinte patrones cubren el 95 % de lo que se usa.
3. **Tipos de línea con su forma completa.** Guardar el patrón entero en vez del primer par en `cad-effective-style.ts:207-227`, y subir `CAD_LINETYPE_SLOT_LIMIT` por encima de 8 — una plantilla de ejecutivo mexicano lleva más de ocho. *Hecho el 2026-09-02 (Ola A · A6); la tabla de patrones de sombreado (punto 2) queda para la siguiente entrega.*
