...
> **Ola G (2026-09-02) — hecho, y medido.** El cimiento se enchufó al dibujo por donde se podía sin tocar el formato: la georreferencia es un MARCADOR —un POINT en la capa GEO con `{geo, crs, east, north}` en `context.metadata`, que el formato ya tiene— y no una tabla nueva (sería decisión del titular). **GEOGRAPHICLOCATION** (alias GEO, MAPCSASSIGN, GEORREFERENCIAR) precisa un punto y su Este/Norte UTM en la zona y el datum elegidos (11N a 16N; WGS84, ITRF92/2008/2020), o con `Geográfica` la latitud y la longitud, que se guardan como el este/norte de la zona que toca porque un grado no es una unidad de dibujo; `Informe` lee sin escribir; volver a georreferenciar reemplaza el marcador. **ID** informa desde entonces «E 660,001.00 N 2,140,001.00 · 19.3477° N, 97.4767° O (WGS 84 / UTM zona 14N, EPSG:32614)». Y **MAPIMPORT** (alias IMPORTARGIS) mete el conjunto EN EL PLANO ABIERTO —lo que la cabecera de `geo-cad-document.ts` decía resolver y `document-import.ts` no hacía—: el `.shp` con sus acompañantes o un `.geojson` por el selector del navegador (varios a la vez, empaquetados en base64 por la puerta de texto del motor, `geo-import-bundle.ts`), o un GeoJSON pegado. `geo-import-plan.ts` decide las cuatro situaciones y las dice antes de escribir: dibujo georreferenciado + archivo con `.prj` → reproyectado vértice a vértice al sistema del dibujo (`crs.ts`) y colocado con su georreferencia (el predio 14N cae en E 660 010 → x = 10 000 mm; una mojonera en grados cae a < 1 mm del marcador); georreferenciado + sin `.prj` → rechazado, porque colocarlo sería adivinar; sin georreferencia + archivo que sabe dónde está → origen local redondo y el dibujo QUEDA georreferenciado con el marcador en (0, 0) (un GeoJSON se proyecta a la zona UTM de su centro); ni uno ni otro → origen local con el aviso de siempre. **Los atributos del DBF llegan al dibujo**: la fila de cada registro va en `context.metadata` de su entidad (unión posicional, y si la tabla no cuadra se declara `geo_attributes_unaligned` en vez de heredar los del vecino). `lib/geo` lee ahora GeoJSON (RFC 7946) a la MISMA forma que el shapefile (`geojson.ts`), con un rasgo sin geometría como nulo para que la fila 3 siga siendo de la geometría 3, y el cuadro «Importar» admite `.geojson` (proyectado a su zona UTM). Specs geojson (34), geo-location (44), map-import (46); **golden 82**. La fila `toolset-map3d` pasa de 0 a 3 de 4 (retiene 1 pt hasta evidencia independiente). Lo que sigue en «todavía no» —NAD27/NAD83, giro de cuadrícula, mapa de fondo, topología, clasificación, exportar a SHP— está en ESCALERA.
...
> **Ola H (2026-09-02) — hecho, y medido.** Medido antes de tocar nada: la entidad `image` se dibujaba como su MARCO y nada más (`fill-entity-adapters.ts` sólo emite caminos; ningún módulo cargaba una textura), IMAGE exigía un `asset://` que ningún código resolvía, y la lámina trazaba el marco con `rasterCommandCount: 0` como literal de tipo. Así que «insertar, escalar, atenuar, recortar, calcar encima» empezaba por VER. **IMAGEATTACH** (alias IAT) elige el archivo por el selector del navegador (canal `image-file`, el mismo reparto que DXFIN y MAPIMPORT), el navegador lo decodifica una vez para saber sus píxeles y lo mete dentro del dibujo como `data:` en `uri` —una cadena que el formato ya admite; tope 8 MB por archivo, dicho en `image-attach-payload.ts`—, y pide inserción, ancho en unidades y giro. Nace `lib/cad/render/image-layer-three.ts`: una malla por imagen debajo de los lotes, con la misma convención de origen flotante y lámina de profundidad NDC, el recorte como polígono triangulado (`THREE.ShapeUtils`) y brillo/contraste/atenuación en el fragment shader con la fórmula de `image-geometry.ts` (50/50 neutro; contraste 0 aplana a gris, brillo 100 es blanco). **IMAGECLIP** (ICL) Nuevo Poligonal/Rectangular y Eliminar guardan el contorno en píxeles (`clipBoundary`); **IMAGEADJUST** (IAD) escribe brillo, contraste y atenuación 0–100 con Restablecer; los tres con `replace` sobre la entidad que ya existía. En la lámina, `paper-space-image.ts` emite el comando `image` con esquinas y recorte en papel y `plot-pdf.ts` lo incrusta con `addImage` girada sobre su esquina inferior izquierda, recorte `W n` y atenuación como estado gráfico; `measureCadPdf` lee ahora los XObject imagen y los `Do`. En el DXF, la imagen embebida sale por su NOMBRE en IMAGEDEF y el manifiesto (`dxf_export_image_embedded_pixels`) dice qué archivo dejar al lado. Specs image-geometry (44), image-layer-three (22), raster-image (75), paper-space-image (29); **golden 83**. Por el camino: `IM` estaba sólo en el descriptor y no en la tabla acad.pgp que consulta el pipeline (como DX en la Ola E). La fila `toolset-raster` pasa de 1 a 3 de 4 (retiene 1 pt hasta evidencia independiente). Lo que sigue en «todavía no» —vectorizar (REM), calibrar, IMAGEFRAME, TRANSPARENCY, un almacén de activos con API, el brillo en el PDF— está en ESCALERA.
...
> **Ola I (2026-09-02) — hecho, y medido.** Medido antes de tocar nada: `dimension-style.ts` tenía cero apariciones de «family»/«subestilo», ninguna cota podía llevar una tolerancia (`dimension-format.ts` sabía formatear «v ± t» desde la fase 73 y nadie la llamaba) y no había un solo normalizado insertable. **Los subestilos por familia** son NOMBRES en la misma tabla, los de AutoCAD (`ISO-25$4` para el radio, `$0` lineal, `$2` angular, `$3` diámetro, `$6` coordenada): `resolveCadDimensionStyle` los resuelve encima del padre cuando se dice para qué tipo de cota, la paleta los aplica al crear la cota (`cadDimensionFamilyStyle`), DIMSTYLE → Aplicar hornea cada cota con su familia y DIMSTYLE → **Familia** crea el subestilo declarando sólo lo que cambia respecto al padre. **DIMTOLERANCE** (alias TOLERANCIA, DTOL) escribe en las cotas designadas su tolerancia simétrica, por desviaciones, por límites o la de un **ajuste ISO 286** (`cadIsoFit`: IT5–IT11, agujeros D–H/JS y ejes d–h/js/k/m/n/p, nominal ≤ 500 mm, calculado sobre lo que mide ESA cota; K/M/N/P se rechazan por la corrección Δ) en `context.metadata`, que el formato ya tiene; `formatCadDimensionMeasurement` rotula «40.00 +0.025/0 mm» y como visor, lámina y DXF consumen ese formateador, sale en los tres; el DXF la lleva además en la XDATA (`tolerance=…`) y vuelve al bolsillo al reimportar. **STDPART** (AMCONTENTLIB, NORMALIZADO, TORNILLO) inserta tornillo ISO 4017, tuerca ISO 4032 y rondana ISO 7089 (M6–M24) y **STEELSHAPE** (AMSTLSHAP2D, PERFIL) la sección de PTR, OC, LI, CPS e IPR por sus medidas, como bloques `MECH-…` con denominación y norma en `description` y la inserción escalada a la unidad del documento; **BALLOON** (AMBALLOON, GLOBO) numera solo y se queda con el bloque; **BOM** (AMBOM, LISTAMATERIALES) lista como TABLE con la posición del globo; **WELDSYMBOL** (AMWELDSYM, SOLDADURA) y **SURFACESYMBOL** (AMSURFSYM, ACABADO) se materializan con la geometría del documento y una marca en metadatos, como el marco de TOLERANCE. Specs dimension-family (38), dimension-tolerance (71), mechanical (84), engine/commands/mechanical (151); **golden 84**. La fila `toolset-mechanical` pasa de 0 a 3 de 4 (retiene 1 pt hasta evidencia independiente). Lo que sigue en «todavía no» —cajetines, AMPOWERDIM, K/M/N/P, DIMTFAC, catálogo IMCA cerrado, entidades `balloon`/`weld`— está en ESCALERA.
