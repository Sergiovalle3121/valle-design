...
**Puntuación (rúbrica 2026-09-06.1).** **Alcance de HOY: 185/213 (86.9 %)** — el flujo diario de dibujo 2D técnico, la cifra que se enseña a un cliente. **Alcance de DESTINO: 246/309 (79.6 %)** — AutoCAD completo con sus verticales, la cifra que mide el camino; lo excluido de hoy es «todavía no», nunca «nunca». 27 pt provienen de evidencia INDEPENDIENTE y 219 pt sólo de evidencia propia; 19 fila(s) retienen 1 pt hasta tener evidencia independiente. 10 de 47 filas están en su tope: Dibujo 2D y precisión, Trabajo ajeno: tomar el plano de otro y trabajar sobre él, Cotas asociativas, HATCH asociativo, MTEXT y texto, Capas y propiedades, Bloques y atributos, Modelo 3D y sólidos B-rep FACETADO, Integridad: el producto hace lo que dice, Capacidad de crecer: las puertas que no se cierran. Una fila sólo llega a su tope cuando TODOS sus criterios verifican, incluidos los que nombran gaps documentados; un gap conocido se declara como criterio que falla, no como nota al pie.

### Núcleo del plano entregable — 113/118

| Categoría | Puntos | Estado | Qué verifica hoy | Qué falta exactamente |
| --- | ---: | --- | --- | --- |
| Dibujo 2D y precisión | 16/16 | Completa | Entidades canónicas (línea, polilínea, círculo, arco, elipse, spline) en el documento, con specs; LINE, PLINE, CIRCLE, ARC, RECTANG, POLYGON, ELLIPSE y SPLINE son tecleables; Coordenadas absolutas, relativas (@) y polares (<) con entrada dinámica; OSNAP sobre el puntero con consulta indexada medida (p95 publicado); ORTHO, rastreo polar y ajustes de dibujo con diálogo aplicable; Conmutadores estándar por tecla de función: F7 (rejilla), F9 (forzado a rejilla) y F12 (entrada dinámica) — F3, F8, F10 y F11 ya existen; Construcción y reparto: XLINE, RAY, DIVIDE, MEASURE, DONUT y REVCLOUD tecleables; Corpus de geometría degenerada (tangencias, radio cero, autointersección, colineales) con criterio publicado por caso | Nada pendiente: todos los criterios declarados verifican |
...
| Xrefs | 5/6 | Parcial | Referencias externas declaradas en el documento canónico; Resolución de recursos y bind con round-trip; XREF, XATTACH, XBIND y XCLIP tecleables — y XATTACH deja la referencia en el documento que recibe el servidor | Las capas de la xref sobreviven: apagar, congelar y atenuar una capa del plano ajeno sin tocar las demás (VISRETAIN) (1 pt) |
...
| Modelado 3D: primitivas, SOLIDEDIT y la cota | 4/5 | Parcial | BOX, WEDGE, CYLINDER, CONE, SPHERE, TORUS, PYRAMID y POLYSOLID tecleables, como UN nodo reeditable cada una, con el volumen medido en papel; SOLIDEDIT con Cara Extruir (nodo push), Cuerpo Comprobar y Cuerpo Separar, y sus otras once ramas declaradas en el propio diálogo; La cota cruza las fronteras del DXF: 30/31, elevación, polilínea 3D y SCU reflejado (lector de terceros como oráculo), y PLINE/RECTANG tecleados dibujan en el plano del SCU inclinado | Con el SCU apoyado en la fachada, el punto del RATÓN sale del plano de trabajo y no del suelo: LINE desde la paleta con dos clics deja el trazo en la fachada, defendido por un golden de navegador (1 pt) |
...
### Integridad y capacidad de crecer — 21/25

| Categoría | Puntos | Estado | Qué verifica hoy | Qué falta exactamente |
| --- | ---: | --- | --- | --- |
| Integridad: el producto hace lo que dice | 13/13 | Completa | Cada comando del registro hace lo que dice: el arnés de veracidad corre en check:cad con CERO éxitos falsos y exenciones declaradas; La interfaz detecta y rechaza el formato en vez de fingir soporte; Ninguna pérdida es silenciosa: el manifiesto de pérdidas viaja con el documento, con el DXF exportado y con la publicación por lotes; Los límites se declaran donde el usuario y el lector los ven: README con límites explícitos, CAPABILITIES del laboratorio con columna de límites, y los anfitriones responden «no disponible» en vez de fingir | Nada pendiente: todos los criterios declarados verifican |
| Capacidad de crecer: las puertas que no se cierran | 8/8 | Completa | La migración aditiva es invariante escrita (ADR-0011) y probada: documentos de esquemas anteriores abren con cero pérdida; La API pública tiene política de versionado y el manifiesto de plugins es formato con versión; El mecanismo de niveles existe: guard genérico por capacidad y catálogo con N entitlements por plan (aunque hoy se venda UNA); El uso por organización se registra desde ya (documentos guardados y publicados, con idempotencia); La puerta única de interoperabilidad está escrita: bytes → neutral → canónico con pérdidas en ambos sentidos; La deuda del monolito está publicada con meta (<8,000 líneas), método por costuras y registro por campaña; el trinquete existe; El mecanismo de corpus independiente está montado: consumo fail-closed con pin y procedimiento de donación publicado en el repo de conformidad | Nada pendiente: todos los criterios declarados verifican |
| Degradación honesta: qué pasa cuando algo se rompe | 0/4 | Ausente | Nada verificado | El fallo permanente no se anuncia como transitorio: un 400 (el documento no cabe nunca) ofrece salida (exportar, reducir) en vez de «espera y vuelve a pulsar Guardar» (1 pt); La profundidad de deshacer prometida es la entregada: un gate versiona undoDepthByTier, la barra de estado la enseña y avisa una vez cuando el historial se poda (1 pt); Ninguna operación deja el documento fuera del límite del servidor: ARRAY, COPY múltiple y DIVIDE piden confirmación por encima de CAD_DOCUMENT_LIMITS.maxEntities (1 pt); El almacenamiento local avisa cuando deja de proteger: openDatabase bloqueado o versión cambiada produce un aviso visible, nunca un recovery colgado en silencio (1 pt) |

### Reconocimiento: se ve y se maneja como AutoCAD — 13/26
...
| La cinta y el gesto: cómo se usa de verdad | 0/6 | Ausente | Nada verificado | Designar un muro, una cota o un bloque abre su pestaña contextual en la cinta, y check-ribbon-coverage la vigila (1 pt); Alt+letra (KeyTips) alcanza cada pestaña y cada panel de la cinta, y ui-command-reach lo cuenta (1 pt); La línea de comandos sugiere mientras se escribe, con el buscador que ya existe (1 pt); El menú del botón derecho depende de lo designado (1 pt); Las pestañas de láminas listan todas, abajo, dicen «Modelo» y Ctrl+RePág/AvPág cambian entre ellas (1 pt); En un dibujo de sólo lectura la cinta conserva los comandos que no mutan nada, y Ctrl+8/Ctrl+9 abren las paletas que AutoCAD abre (1 pt) |
| Sin ratón, sin vista y en dos idiomas | 0/6 | Ausente | Nada verificado | El gate de axe del estudio falla con impacto moderate, no sólo serious\|critical, y el estudio tiene su encabezado (1 pt); Ningún control se enfoca sin verse: el trinquete de foco visible está en cero (1 pt); Cada panel de la cinta tiene role y aria-label, y la cinta se navega por flechas con un golden que lo defiende (1 pt); El prompt vivo se puede volver a leer: aria-describedby en la caja, el log recibe foco y F2 existe (1 pt); forced-colors y prefers-contrast existen, con la misma disciplina que prefers-reduced-motion (1 pt); El vocabulario de comandos en español que la promesa comercial declara existe, se prueba y no choca destructivamente con los alias ingleses de AutoCAD (1 pt) |

### Los siete toolsets de AutoCAD — 21/28
...
| Toolset Raster Design (mitad útil) | 3/4 | Parcial | Insertar, recortar por polígono y ajustar (brillo/contraste/transparencia) un plano escaneado; Vectorizar líneas y textos de un escaneo a entidades | Nada pendiente: todos los criterios declarados verifican |
...
| Toolset Electrical | 3/4 | Parcial | Instalación eléctrica en PLANTA: símbolos de planta, numeración de conductores y etiquetado de componentes derivados del dibujo (sin símbolos de esquema de control: ni bobina, ni contacto, ni contactor, ni guardamotor); Informes: listado de conductores, cuadro de cargas en el dibujo y revisión contra la NOM | Nada pendiente: todos los criterios declarados verifican |
| Toolset Plant 3D | 3/4 | Parcial | Diagramas P&ID: catálogo de equipos etiquetados y líneas numeradas, todo derivado del dibujo; Tubería 3D por especificación e isométricos | Nada pendiente: todos los criterios declarados verifican |

### Comprar, crecer, entrar y salir: lo que decide un despacho — 3/12

| Categoría | Puntos | Estado | Qué verifica hoy | Qué falta exactamente |
| --- | ---: | --- | --- | --- |
| Compra y crecimiento del despacho | 2/3 | Parcial | Checkout con tarjeta, OXXO y SPEI, precios en céntimos enteros servidos del catálogo de la base de datos; Cuando el período termina la cuenta pasa a sólo lectura y sigue abriendo, imprimiendo y exportando: obligación del servicio probada contra PostgreSQL | Los asientos se comprueban en el servidor y el despacho puede COMPRAR el siguiente sin salir del producto: choca contra el límite, compra el cuarto, invita (1 pt) |
| Facturación declarada con su límite | 1/3 | Parcial | Reembolsos y disputas llegan por webhook firmado y dejan asiento verificable | «Factura CFDI» sólo se anuncia cuando el proveedor de timbrado no está en modo manual: el sello de la portada y la respuesta del FAQ se derivan del descriptor, con una prueba que lo impide en modo manual (1 pt); Cada plantilla de correo que el árbol encola tiene renderizador: un gate de cobertura recorre todos los nombres encolados y ninguno se descarta en silencio (1 pt) |
| El expediente que pide un comprador | 0/3 | Ausente | Nada verificado | El cliente puede leer quién tocó sus planos: GET /v1/organizations/{id}/audit-log con pantalla, aislamiento por inquilino probado, retención y purga declaradas (1 pt); El SLA se publica y sus nombres de plan son los del catálogo vendible, atados por una prueba (1 pt); Exportación de datos personales y borrado de cuenta (derechos ARCO/RGPD) con contrato, ruta y prueba (1 pt) |
| Migración de entrada y de salida | 0/3 | Ausente | Nada verificado | Del estado vacío o de /demo al documento abierto en la cuenta nueva CON el dibujo dentro: arrastrar-y-soltar un DXF, proyecto implícito, returnTo que sobrevive a la verificación (1 pt); El sitio dice —y una prueba defiende— que /demo abre el DXF del propio visitante sin cuenta, sin instalar y sin subir nada (1 pt); La salida completa: el DXF del API público sale con discriminación por tipo y manifiesto de pérdidas, y la organización entera se puede exportar (1 pt) |

### Lo que sólo un CAD en el navegador puede dar — 0/10

| Categoría | Puntos | Estado | Qué verifica hoy | Qué falta exactamente |
| --- | ---: | --- | --- | --- |
| El plano entregado tiene dirección | 0/4 | Ausente | Nada verificado | Publicar produce una URL que el cliente abre en el teléfono sin cuenta ni instalación y que enseña la LÁMINA con su versión, su sha256 y sus pérdidas declaradas (2 pt); El enlace de la lámina caduca y se revoca: tras revocar, 403, probado de extremo a extremo sin sesión (1 pt); El enlace de la lámina va impreso como QR en el cajetín: el plano de papel de la obra apunta a su versión viva (1 pt) |
| El invitado existe entre máquinas | 0/2 | Ausente | Nada verificado | El invitado del enlace aparece en la presencia del autor con su cursor y su encuadre, y al cliente nunca se le afirma que no hay nadie mirando cuando sí lo hay (1 pt); El invitado puede entrar en la llamada que ya está dentro del plano (1 pt) |
| Un punto del dibujo tiene URL | 0/2 | Ausente | Nada verificado | Una URL lleva a un punto y un encuadre concretos del dibujo, en el estudio y en el enlace de revisión (1 pt); Al comentario del cliente se le puede contestar desde el estudio y el autor se entera de que llegó (1 pt) |
| Se instala y avisa con la pestaña cerrada | 0/2 | Ausente | Nada verificado | La aplicación se instala (manifiesto, service worker, criterios de instalabilidad) y una prueba lo defiende (1 pt); Con la pestaña cerrada, el producto avisa: xref cambiada, bloque de biblioteca redefinido, comentario del cliente (1 pt) |
