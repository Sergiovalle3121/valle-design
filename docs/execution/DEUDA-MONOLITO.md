...
| 2026-08-31 | cinta de interfaz | 18,921 | 135 | Partió ya en 19,002/135 —una campaña paralela bajó el `useState` de 140 a 135 entre el 29 y el 31 sin dejarlo escrito aquí; esta fila sólo corrige el registro, no se atribuye esa baja—. Lo que sí sale en esta campaña: la barra de estado y los conmutadores (candidato §2 de este documento, ~216 líneas) → `components/cad/studio/CadStatusBar.tsx`, con sus ~40 dependencias agrupadas en 7 objetos con nombre (`diagnostics`, `saveState`, `layersInfo`, `validation`, `misc`…) — por debajo del umbral de "extraer la vista" que este mismo documento calibra. `CadStatusBar` es presentación pura: no poseía estado propio, así que el `useState` se queda en 135. El hueco de líneas liberado montó la cinta (`components/cad/ribbon/`), el ViewCube/barra de navegación (`components/cad/viewport/CadViewCube.tsx`, `CadNavigationBar.tsx`) y un botón genérico "Terminar comando" que le faltaba a todo comando del motor invocado sin pasar por la paleta vieja. **−81 líneas** netas pese a las cuatro superficies nuevas montadas. |
