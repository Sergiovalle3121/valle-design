...
| `check:command-integrity` | Todos los comandos del registro: ninguno responde éxito sin efecto verificable; exenciones declaradas y bidireccionales. | O tu comando nuevo termina en silencio/afirmando sin efecto (arréglalo: que haga o que diga que no puede), o la sonda no sabe conducirlo (decláralo en `command-integrity-exemptions.json` como `{razon, spec, conduce, comprueba}`: el gate exige que ese spec de `src/**` exista, conduzca el comando con entradas que no sean sólo cancelar y asierte su efecto en las 30 líneas siguientes). |
